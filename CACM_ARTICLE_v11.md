...
Two regimes appear in the proof. For patterns whose effective alphabet is smaller than Σ — the vast majority of production regex — a cyclic-group witness in the syntactic monoid of *L_decode(p)* directly demonstrates non-aperiodicity, hence non-membership in AC⁰. For patterns whose alphabet equals Σ (regex containing `.` or negated character classes like `[^>]*`), the cyclic witness does not directly apply; the bypass still succeeds operationally because substring matching breaks under any sufficiently disruptive interleaving, but the *structural* patchability claim there requires an alphabet-extension argument we sketch in Appendix A and treat formally in the technical paper. We are explicit about this regime split rather than universalize.
...
We do not claim that all AI security is mathematically impossible. We do not claim neural guardrails or LLM-judges have the same blind spot — they do not, because TC⁰ is strictly larger than AC⁰. We do not claim the bound applies to embedding-based similarity filters, fine-tuned classifiers, model-internal safety training, behavioral / rate-limit guardrails, or any guardrail outside the star-free string-matching class — those are explicitly out of scope (§3 taxonomy). We do not claim full structural patchability for the regime where a guardrail's regex alphabet equals Σ (regex containing `.` or negated character classes); the operational bypass works there, but the *no-finite-patch-closes-it* claim in that regime requires an alphabet-extension argument we sketch but do not fully formalize. We do not claim a regulatory framework follows from this result.
...
*Regime 2 (alphabet condition fails).* If no such *f* exists in Σ (regex contains `.` or negated character classes whose minimal-DFA alphabet equals Σ), the cyclic-witness construction does not apply directly. The bypass still succeeds *operationally* — interleaving with any character disrupts the contiguous substring match — but the *patchability* claim (no further aperiodic regex closes the gap) requires extending the syntactic-monoid argument. We sketch an alphabet-extension approach in `paper/main.tex` (introduce a fresh symbol ★ ∉ Σ, prove the cyclic structure over Σ ∪ {★}, transfer back via congruence morphism) but do not give it as a complete proof; the structural patchability of Regime 2 is therefore conjectured rather than fully established here. We note that production guardrail patterns containing `.` or `[^X]*` are common (e.g., `<script[^>]*>`); for these the empirical bypass succeeds against the original guardrail, and the patchability question remains open in the strict sense.
