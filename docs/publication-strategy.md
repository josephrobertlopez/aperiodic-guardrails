...
- 142 production patterns audited across 11 open-source guardrail tools (LLM Guard, Rebuff, NeMo Guardrails, Guardrails AI, llm-guard-py, Presidio, GitLeaks, OWASP-LLM, WAF-generic, LangKit, BodAIGuard); 100% aperiodic. Aperiodicity verified computationally on each pattern's enumerated monoid (paper §8.1, methodology steps 3–4), not by syntactic template-matching.
...
- Three case studies: aspirin synthesis, TCP state machine, Zerbik calculus (synthetic, constructed for the paper to control for memorization).
...
- Four formally distinct barriers: algebraic (unconditional), information-theoretic (illustrative under uniformity), computational (NP-hard via 3-SAT, with hardness in fiber-search not single-evaluation), structural (functoriality + opacity).
...
- Artifacts: `monoid-extractor` (~860 LOC Python), `secrets-router` (MIT-licensed MCP server, ~800 LOC, addresses the credential-exfiltration subclass of attacks), 376-line PoC.
...
- Presenting "100% aperiodic" without noting that aperiodicity was verified computationally rather than by syntactic template — the corpus-bias question is the first one a reviewer asks.

## 3. Pre-submission process checklist

### 3.1 Critical-path items (block all external pitches)

These three items run serially in effect because each is a prerequisite for credible external claims. Plan for ~6–12 weeks total lead time before the first pitch goes out.

1. **arXiv endorsement.** Identify a cs.CR or cs.FL endorser; submit; wait for clearance. Days to weeks. Required before any external "arXiv version" link — without this, the pitch's companion-paper link will not resolve.
2. **Responsible disclosure.** Notify ProtectAI of the LLM Guard `BanSubstrings` 0% detection result. Standard 30–90 day window before public claim. Pin the tested version in any external statement. Longest-pole item; required for venues with explicit ethics expectations (USENIX Security, IEEE S&P, NDSS).
3. **Bibliography audit.** Verify every entry in `references.bib` resolves to a real paper. **Specific items requiring action**:
   - Three entries currently with author "Anonymous" and arXiv IDs in the 2026 range (e.g., `s2c2026`, `proofofguardrail2026`, `concretization2025`). Each must either resolve to a real paper (citation updated to non-anonymous form) or be removed. Hostile reviewers in 2026 read "Anonymous" + arXiv ID as a possible hallucinated-citation signature.
   - Spot-check at least one citation per topic cluster (algebraic, jailbreak/encoding, transformer-expressivity, IS/security).

### 3.2 Parallelizable items (run alongside critical path)

4. **README reconciliation.** README's empirical numbers must match `paper/main.tex` §8.2 (BFS / random-beam / ToT yields). The "ToT 4.3× BFS" comparison line is stale and must be replaced with the N=50 numbers. Also fix the abstract sentence that conflates V3 with a search strategy.
...
- **Path A (technical-primary).** USENIX Security or IEEE S&P primary; CACM Practice as follow-up after acceptance. No editor-relationship dependency. Highest probability of landing on technical merit. **Recommended baseline.**
- **Path B (cross-disciplinary).** CACM Practice as primary requires either an existing editorial-board relationship or a co-author with one. Without that, the access barrier and curation latency make this a high-variance bet. Consider only if such a relationship is in hand.
- **Path C (parallel).** ACL LLMSec workshop in parallel with longer-venue submission. Fast publication beachhead while the primary submission is in flight.

The risk in Path A is missing cross-CS reach until a follow-up — manageable. The risk in Path B is committing 3+ months to a curated venue with no editorial relationship and then needing to recraft for technical venues if it stalls — substantial. Path C de-risks by establishing a publication beachhead quickly.
...
guardrail tools; 100% aperiodic (verified computationally per-pattern, not by
syntactic template). End-to-end testing against ProtectAI's LLM Guard
...
Verify dates and details against primary sources before citing externally. For 2026 publication windows, refresh with current-quarter incidents in addition to the foundational set.
...
1. **Primary venue.** USENIX Security / IEEE S&P (Path A) vs. CACM Practice (Path B, conditional on editorial relationship). Default: Path A.
...
6. **Bibliography audit timeline.** Block all external pitches on completion (current default) vs. proceed with internal materials in parallel.
