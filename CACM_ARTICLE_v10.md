...
### Six Decades of Algebra, Twelve Production Libraries, And the Whole AC⁰ Tier of Your LLM Stack
...
This is not an engineering bug. It is a *mathematical property* of substring-matching regex, sitting in the formal-language-theory literature since Schützenberger 1965. The property applies — provably, unconditionally, with no unproven complexity-class separations — to every aperiodic-monoid regex guardrail you have ever deployed.

It is not patchable inside the regex tier. The union of finitely many aperiodic-monoid languages is itself aperiodic, which means your patched guardrail still lives in **AC⁰**, the constant-depth circuit class that *provably cannot count modulo a prime*. The bypass class — interleaving a payload with a filler at every other character position — exploits exactly that limit.
...
The same construction works against:

- **Outbound DLP regex** scanning model output for PII or secret patterns. The model produces MOD_p-encoded content; the regex passes it; the consumer (human or downstream system) decodes.
- **Tool-call argument validators** doing regex whitelisting on agent function calls, where the downstream tool processes a string that another model later decodes (common in multi-LLM agentic chains).
- **JSON Schema `pattern` fields** validating structured outputs, where the consuming system passes the validated-but-encoded value to a model.
- **Trie- and DFA-based blocklists** matching against finite literal sets — finite languages are trivially aperiodic, so the same algebraic class applies.
...
| **In scope** (AC⁰ string-matchers) | **Out of scope** (TC⁰ or different class) |
|---|---|
...
| JSON Schema `pattern` field validators | Behavioral / rate-limit / quota guardrails (orthogonal to content) |
| Trie- and DFA-based blocklists matching finite literal sets | Model-internal safety (RLHF, constitutional AI, refusal training) |
| Bloom-filter blocklists (with the usual false-positive caveat) | Token-budget / context-window guardrails |
| **Any composition** (intersection or union) **of the above** | Formal verification of model behavior on bounded input classes |
...
| **Downstream LLM in agent chain** | Same as input case — decodes | **High** |
| **Tool execution layer** (shell, DB, FS) | Processes literally; usually fails to execute encoded form | **Low** (formally bypassed but operationally inert) |
| **Schema-consumer pipeline** | Processes literally | **Low** |
...
We do not claim that all AI security is mathematically impossible. We do not claim neural guardrails or LLM-judges have the same blind spot — they do not, because TC⁰ is strictly larger than AC⁰. We do not claim the bound applies to embedding-based similarity filters, fine-tuned classifiers, model-internal safety training, behavioral / rate-limit guardrails, or any guardrail outside the AC⁰ string-matching class — those are explicitly out of scope (§3 taxonomy). We do not claim a regulatory framework follows from this result.
...
### A.4 No patch within the AC⁰ class fixes it

The union and intersection of finitely many aperiodic-monoid regular languages are aperiodic (closure of star-free under Boolean operations and concatenation). Therefore any aperiodic *G'* added to *G* — at any tier of a multi-tier pipeline — leaves the system in AC⁰, which cannot decide *L_decode(p)*. This generalizes the patch-resistance from "no further regex" to "no further AC⁰ string-matcher of any kind." Vendors *can* patch any individual filler choice; the bound asserts only that no finite collection of such patches covers all filler choices simultaneously.
