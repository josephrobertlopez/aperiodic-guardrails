...
$h_+, h_- \in \Sigma^k$ of equal length $k \ge 1$.
...
\emph{Choosing $h$.}
Set $h = h_+ \in L(r)$ (a string in $L(r)$).
Then $\sigma_0(h)$ extracts the characters of $h$ at positions $0, p, 2p, \ldots$,
which for $h$ of length $\ge p$ contains $h[0]$.
For $j > 0$, $\sigma_j(h)$ extracts the characters of $h$ at positions
$(-j \bmod p), p + (-j \bmod p), \ldots$, which differs from $\sigma_0(h)$ in the
specific characters of $h$ that appear (the offset is shifted by
$-j \bmod p$).
By the hypothesis that $L(r)$ contains both $h_+$ and $h_-$ differing at some
position $r$, we may choose $N$ large enough that the offset shift between
$\sigma_j$ and $\sigma_k$ ($j \neq k$) places $h_+$ at one offset and $h_-$ at
another, ensuring the resulting decoded substrings differ in at least one
character at a position relevant to membership in $L(r)$.

For the inequivalence, it suffices to find any $L_{\mathsf{decode}}(p)$-deciding
context distinguishing $u_j$ from $u_k$.
For substring-matched $L_{\mathsf{blocked}}$, this reduces to finding a context
in which the decoded substring $\sigma_j(h^N)$ contains a substring matching
$L(r)$, while $\sigma_k(h^N)$ does not (or vice versa).
Such a context exists for any $h$ of length $\ge p$ with at least two distinct
characters at positions $r, r + p \pmod k$, because the offset shift between
$\sigma_j$ and $\sigma_k$ permutes which characters appear at the substring-match
position.
The hypothesis that $L(r)$ contains two distinct strings of equal length
guarantees the existence of such an $h$.
