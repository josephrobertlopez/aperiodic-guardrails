...
transformers in $\mathrm{TC}^0$~\cite{merrill2023saturated,chiang2024tighter,merrill2024survey},
...
A pattern such as $(aa)^*$, which recognizes strings of even $a$-count,
requires $\mathbb{Z}/2\mathbb{Z}$ in its syntactic monoid and falls
outside this syntactic scope.
...
and let $\b$ denote the word-boundary anchor whose semantics, as a position predicate
on $\Sigma^*$, expand to $(?{<}{=}\W)\cup(?{<}{!}\w)$ at the start position and the
mirror condition at the end (where $\w = [\mathrm{A\text{-}Za\text{-}z0\text{-}9\_}]$ and
$\W = \Sigma \setminus \w$).
Then $L_r' := \Sigma^* \cdot \b \cdot L(r) \cdot \b \cdot \Sigma^*$ has aperiodic
syntactic monoid.
\end{lemma}
\begin{proof}
Define the auxiliary languages
$B_L = \{w x : x \in L(r),\, w \in \Sigma^*\,\text{and the last character of}\ w
\text{is in}\ \W\ \text{or}\ w = \varepsilon\}$ and analogously
$B_R$ for the right anchor.
Each is a finite intersection of $\Sigma^* \cdot L(r) \cdot \Sigma^*$ with a
two-character context predicate (``character at position $|w|$ is in $\W$ or
...
$L_r' = \Sigma^* \cdot \b \cdot L(r) \cdot \b \cdot \Sigma^*$ is the
...
$\b$ anchors that fall outside the syntactic grammar of
Theorem~\ref{thm:substring-aperiodic}.
...
$\b[\mathrm{A\text{-}Z}]{2}\d{2}[\mathrm{A\text{-}Z0\text{-}9}]{4}\d{7}([\mathrm{A\text{-}Z0\text{-}9}]?){0,16}\b$---is
...
transformers sit in $\mathrm{TC}^0$~\cite{merrill2023saturated,chiang2024tighter},
