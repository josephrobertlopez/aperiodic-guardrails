...
Hagendorff et al.~\cite{reasoning_jailbreak2026} demonstrate that large reasoning models
...
\emph{Direct argument.} The decoded sequence $w[0::p]$ is precisely the
projection of $w$ onto positions $\equiv 0 \pmod{p}$, and computing the
position residue requires the $\mathrm{MOD}_p$ predicate. By
Theorem~\ref{thm:fss} (Furst--Saxe--Sipser),
$L_{\mathsf{decode}}(p) \notin \mathrm{AC}^0$. Non-aperiodicity of
$M(L_{\mathsf{decode}}(p))$ follows from the BCST characterization
(Theorem~\ref{thm:bcst}). This avoids any minimal-DFA assumption on the
...
Theorem~\ref{thm:blindness} covers Regime~1 (patterns with alphabet properly
smaller than $\Sigma$ --- the vast majority of production regex). For the
residual class of substring-matched patterns whose minimal-DFA alphabet equals
$\Sigma$ (e.g., \texttt{<script[\^{}>]*>}), the algebraic blindness conclusion
is established empirically by direct monoid enumeration in
\S\ref{sec:corpus}, which decides aperiodicity per-pattern without relying on
the syntactic-template structure assumed by Theorem~\ref{thm:blindness}'s
alphabet condition.
\end{remark}
...
$P_e \ge h^{-1}(0.3) \approx 0.053$ (about $5\%$).
...
\begin{itemize}
  ...
  \item Guardrail $G\colon R^n \to \{0,1\}$ is the Boolean circuit encoding
    ...
    $\varphi(\iota(v_1), \ldots, \iota(v_n)) = \mathsf{true}$.
    The existential quantifier over interpretations $\iota$ in the \textsc{GIDP}
    definition supplies the SAT search.
\end{itemize}
...
\begin{remark}[Trivialization under the algebraic restriction]\label{rem:gidp-trivial}
The reduction above encodes an arbitrary 3-CNF formula as the guardrail $G$.
For guardrails in the substring-matching class addressed by this paper (i.e.,
$G \in \mathrm{AC}^0$), the 3-CNF encoding falls outside the class ---
Proposition~\ref{thm:np} is therefore an upper bound on the complexity of
generic guardrails, not a tight bound for the algebraic class we address.
Within the algebraic class, the GIDP question reduces to checking whether the
encoded payload satisfies the star-free pattern, which is decidable in linear
time (regex match). The NP-hardness statement is preserved here for
completeness; readers should not interpret it as a hardness result for the
algebraic-blindness deployment context.
\end{remark}
...
V3 is presented as a \emph{structural challenge for inference-layer defenses}, not
as an LLM-bypass attack in the conventional sense.
The construction places the attacker in possession of an offline interpretation
table $I\colon M \to R$ while the LLM is asked only to solve a formally specified
grammar derivation problem over abstract symbols.
The content visible at the prompt boundary is, by construction, indistinguishable
from a legitimate formal-reasoning exercise: no inference-layer tier---regex,
neural classifier, or any composition thereof---can separate adversarial from
benign instances at that boundary, because the harmful semantics live entirely
in the table $I$ that the inference layer never sees.
The empirical results in Section~\ref{sec:tot-benchmark} reinforce this framing
rather than undermine it: as we report below, exhaustive grammar search dominates
LLM-guided variants on small benchmarks, which is consistent with the LLM
\emph{not} being the attack surface here---the leverage comes from the abstraction
grammar and the offline interpretation table, and defense for this class of
attack must live at the execution layer (Section~\ref{sec:secrets-router}), not
the inference layer.

Concretely: the attacker constructs a context-free grammar $\mathcal{G}$ over
abstract symbols $M$, representing the procedure to be derived.
...
\paragraph{Interpretation.}
The headline finding is that exhaustive grammar search outperforms LLM-guided
variants on these small benchmarks.
This is a feature of the V3/V4 framing, not a flaw in it: V3 and V4 are
structural challenges for inference-layer defenses, not LLM-specific bypasses,
and the LLM is not the attack surface.
Mechanically, at the scale of these grammars (mean 87 states explored) BFS is
cheap, complete, and optimal, and the symbol-only grammars
...
effectively noise with respect to reachability, and their $0.6\times$ composite
weight actively misdirects the ToT beam into the high-yield trap paths.
The honest reading: LLM-guided search adds value only when symbols carry
meaning the model can attach to---the regime the aspirin and TCP case studies
of \S\ref{sec:aspirin} exhibit---whereas on fully opaque synthetic grammars
classical search wins.
The security claim of V3 does not depend on ToT outperforming BFS on such
benchmarks; it requires only the \emph{existence} of a solver producing valid
derivations, which all three strategies demonstrate on solvable grammars.
The leverage exploited by V3/V4 is the abstraction grammar plus the offline
interpretation table~$I$, which together render the prompt-boundary content
indistinguishable from legitimate formal reasoning---an inference-layer
tier cannot tell the two apart because $I$ never crosses that boundary.
Defense for this class belongs at the execution layer
(Section~\ref{sec:secrets-router}).
...
\item \textbf{Parallel with neural breaks the ceiling (capacity claim):}
  ...
  depth or precision scales with sequence length (the precision angle
  is sharpened in Chiang-Cholak-Pillay~\cite{chiang2023tighter}). The empirical
...
\item \textbf{Critical blend weight.}
  ...
  (neural detects), the blend detects $\phi_p(x)$ if
...
This work was developed using an agentic AI research pipeline.
The structural argument (Schützenberger~$\to$~BCST~$\to$~Furst--Saxe--Sipser)
is a chain of classical results assembled by the author; the
substring-aperiodicity lemma and its proof were drafted iteratively with AI
assistance and verified by hand against Pin's \emph{Varieties of Formal
Languages} and the BCST original.
The 142-pattern corpus, the monoid extractor (\texttt{tools/audit\_v14.py}),
the MOD\textsubscript{p} bypass harness, and the empirical pilots were
implemented and run by the author.
The article and its companion preprint went through multiple
adversarial-review cycles using AI peer-reviewer agents; every numerical
claim, citation, and inline \texttt{(verified: $\ldots$)} anchor was
re-checked against the released artifacts before submission.

\paragraph{Pipeline composition.}
Orchestration and audit ran on Anthropic Claude Opus~4.7 (1M context); local
code generation was delegated to a code-monkey subagent backed by Ollama
\texttt{qwen2.5-coder:14b} on the author's GPU, with Claude Sonnet
handling multi-step subagent coordination.
The preprint passed through twenty-two named article revisions (CACM v1
through v22) and at least ten adversarial peer-review cycles via a dedicated
peer-reviewer agent persona; each cycle produced a written reviewer report,
a separate author-defense pass, and an arbitration decision logged to the
project repository.

\paragraph{Hand-verified anchors.}
The following classical results were re-derived or re-checked against
canonical sources by the author rather than taken on AI authority:
Pin~1986, \S I.4 (closure of star-free languages under Boolean operations);
Barrington--Compton--Straubing--Th\'erien~1992 (aperiodic
$\Leftrightarrow\mathrm{AC}^0$); Furst--Saxe--Sipser~1981
($\mathrm{AC}^0 \not\supseteq \mathrm{MOD}_p$); Hahn~2020 (TACL,
parity-detection scaling for transformer architectures).

\paragraph{Failure modes observed and corrected.}
The pipeline produced---and the audit cycle caught---the following failure
classes during development:
(i)~citation errors, including multiple author misattributions (Yue~$\to$~Li,
Xu~$\to$~Hackett) and venue misattributions (Chiang~ICML~2023, not
ICLR~2024);
(ii)~overclaim patterns, most notably an early universal-impossibility claim
that was retracted after a parity-projection counterexample was constructed
during adversarial review;
(iii)~mathematical hand-waves caught and tightened, including a redundant
intermediate (Theorem~6.6 in an earlier draft) that was cut, and a
minimal-DFA assumption in Theorem~\ref{thm:blindness} step~(3)
that was replaced with a direct $\mathrm{AC}^0\,/\,\mathrm{MOD}_p$ argument;
(iv)~empirical scope creep, where an early ``9 of 12 sources'' formulation
was reframed to the more defensible ``WAF-generic plus adversarial test
suites; every library examined ships at least one such matcher.''

\paragraph{Re-verification methodology and disclosure.}
Every numerical claim in this preprint is traced to a JSON artifact under
\texttt{results/}; every citation was cross-checked against arXiv, the
publishing venue, or (for textbooks) the canonical edition.
The author retained final authority over the structural argument framing,
mathematical correctness, calibration of the AAF-style discipline rules, and
all ship decisions; the AI pipeline acted as drafter, code-monkey, and
adversarial reviewer, never as arbiter.
The monoid extractor, all bypass harnesses, and all empirical pilot JSON
outputs are released as part of the artifact bundle deposited at
DOI~\texttt{10.5281/zenodo.20103493}.
