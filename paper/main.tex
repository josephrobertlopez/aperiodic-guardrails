...
We establish \emph{algebraic} (unconditional) blindness for modular
counting attacks (Theorem~\ref{thm:blindness}), and---under an explicit
semantic-opacity assumption on symbol naming---\emph{string-level}
indistinguishability for the homomorphic-reasoning construction
(Observation~\ref{thm:indistinguish}).
Ball et al.\ give a more general computational result under cryptographic
assumptions; our algebraic result is assumption-free but applies to the
specific regex-guardrail setting, and our indistinguishability observation
is assumption-light (symbol opacity only) but applies to the specific
...
covertext.
Observation~\ref{thm:indistinguish} can be viewed as establishing
\emph{perfect} steganographic security for the homomorphic reasoning
channel \emph{under Assumption~\ref{ass:opacity}}: any distinguisher
operating on strings has exactly zero advantage (not merely negligible),
because the attack's LLM-visible content lies as a string set inside the
legitimate formal-reasoning task set, making the
