...
For the patchability claim, we exhibit an explicit $\mathrm{AC}^0$ many-one
reduction from $\mathrm{MOD}_p$ to $L_{\mathsf{decode}}(p)$.
Assume $L_{\mathsf{blocked}}$ is non-trivial: there exist strings
$h^+, h^- \in \Sigma^+$ of equal length $\ell$ with
$h^+ \in L_{\mathsf{blocked}}$ and $h^- \notin L_{\mathsf{blocked}}$
(any production guardrail satisfies this; if it does not, it accepts every string
or no string and is uninteresting).
Given an instance $x = x_1 x_2 \cdots x_n \in \{0,1\}^n$ of $\mathrm{MOD}_p$,
construct $\rho(x) \in \Sigma^*$ as follows.
Let $\hat h(b) = h^+$ if $b = 1$ and $\hat h(b) = h^-$ if $b = 0$.
Place $\hat h(x_i)$ at positions $\bigl[(i-1) \cdot p \cdot \ell,\; i \cdot p \cdot \ell\bigr)$
where the $\ell$ characters of $\hat h(x_i)$ occupy the
positions $\equiv 0 \pmod p$ within that block (i.e., one $\hat h$-character at every
$p$-th position) and a fixed neutral filler $f \in \Sigma$ fills the
remaining $(p-1) \cdot \ell$ positions.

This map $\rho$ is computable in $\mathrm{AC}^0$ (each output position depends on
the $\mathrm{AC}^0$-decidable membership $i \cdot p \cdot \ell \le \cdot < (i+1) \cdot p \cdot \ell$
and the constant indexing into $h^+, h^-$, both AC$^0$-computable since $p, \ell$
are constants of the construction).
By construction, $\rho(x)[0::p]$ is the concatenation of $\hat h(x_1), \ldots, \hat h(x_n)$,
and this concatenation lies in $\bigcup_{k} L_{\mathsf{blocked}} \cdot (\Sigma^*)^k \cdot \Sigma^*$
(i.e., contains $h^+$ as a substring) iff at least one $x_i = 1$.
A simple variation produces a reduction whose output lies in $L_{\mathsf{decode}}(p)$ iff
$\sum_i x_i \equiv 0 \pmod p$: choose $h^+, h^-$ such that the boundary between consecutive
$\hat h(x_i)$ blocks creates a substring in $L_{\mathsf{blocked}}$ exactly when the parity
condition is satisfied (constructible because $L_{\mathsf{blocked}}$ has at least one
accepted string, which can be embedded across block boundaries).
Therefore an $\mathrm{AC}^0$ decider for $L_{\mathsf{decode}}(p)$ would yield an
$\mathrm{AC}^0$ decider for $\mathrm{MOD}_p$, contradicting Theorem~\ref{thm:fss}.
It follows that no language in $\mathrm{AC}^0$ --- and hence no aperiodic-monoid
regex guardrail $G'$ --- can decide $L_{\mathsf{decode}}(p)$.
\end{proof}
...
\begin{remark}[Patterns where the alphabet condition fails]\label{rem:alphabet-condition}
For patterns whose minimal-DFA alphabet equals $\Sigma$ (e.g., patterns containing
$.$ or negated character classes such as \texttt{[\^{}>]*}), no $f \in \Sigma$
satisfies the self-loop hypothesis of step~(2), so the cyclic-witness construction
in $M(L_{\mathsf{decode}}(p))$ does not apply directly.
However, step~(3) of Theorem~\ref{thm:blindness} does not require step~(2)'s witness:
any aperiodic regex guardrail $G'$ added to $G$ has $L_{G'} \in \mathrm{AC}^0$
(Theorem~\ref{thm:bcst}), and deciding membership in $L_{\mathsf{decode}}(p)$
requires identifying which positions of the input lie at index
$\equiv 0 \pmod{p}$ --- a $\mathrm{MOD}_p$ predicate on the position counter ---
which $\mathrm{AC}^0$ provably cannot compute (Theorem~\ref{thm:fss}).
The blindness conclusion therefore holds for these patterns by step~(3) alone.
The empirical bypass for full-alphabet patterns succeeds operationally because
substring matching breaks under any sufficiently disruptive interleaving (a generic
property of substring matching, not the algebraic bound); the patchability conclusion
is what step~(3) adds beyond this generic observation.
\end{remark}
