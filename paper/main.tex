...
\texttt{\textbackslash{}b[A-Z]\{2\}\textbackslash{}d\{2\}[A-Z0-9]\{4\}\textbackslash{}d\{7\}([A-Z0-9]?)\{0,16\}\textbackslash{}b}---is
aperiodic by Lemma~\ref{lem:word-boundary} applied to its inner regex,
...
$s \in \{0, 1, \ldots, k-1\}$ with $\gcd(p, k) \mid s$ but $s \not\equiv 0 \pmod{k/\gcd(p,k)}$
(equivalently: the differing position survives the modular periodic decimation by $p$).
...
The strengthened hypothesis is satisfied by every nontrivial production guardrail we
have surveyed: in practice $L(r)$ contains witness strings that differ at multiple
positions, so the gcd-and-position condition holds for the prime $p$ of any practical
bypass. Patterns where $L(r)$'s accepted/rejected strings differ only at positions
periodic mod $k/\gcd(p,k)$ are pathological corner cases (e.g., $L(r) = a^k \cup b^k$
for $\gcd(p, k) = k$); we do not claim the theorem covers these.
\end{theorem}
...
Let $h_+, h_- \in L(r)$ of equal length $k$ differ at position $s$ satisfying
the gcd hypothesis of the theorem statement.
Choose $N$ such that $Nk \ge p \cdot \mathrm{lcm}(p, k)$; this guarantees both
$\sigma_j(h_+^N)$ and $\sigma_j(h_-^N)$ each contain the position-$s$ character
of their respective $h$ at some decoded index, and the periodic decimation by
$p$ preserves the inequality at position $s$ across $\sigma_0, \sigma_1, \ldots, \sigma_{p-1}$.
...
By the gcd hypothesis on $s$, the position $s$ appears in this index set for
some $t$ with $\sigma_j$, and at a different index $t'$ with $\sigma_k$.
...
this work. Pilots whose detail belongs to the companion CACM v14 article
(\S\ref{sec:corpus} and Pilot~6 above are the paper-side empirical legs)
are archived but not reproduced here in float-table form: Pilot~B
(filler-diversity bypass; same 14 instances $\times$ 4 primes $\times$ 7
fillers $\to$ 392/392 bypass) is in
\texttt{results/printable\_filler\_bypass.json} and reported in CACM v14
Appendix~B.3; Pilot~C (LLM decode reliability with random-character
control; 4 open-weight models $\times$ 60 decode cells $\times$ 20
control cells $\to$ 91.7\,pp lift) is in
\texttt{results/llm\_decode\_pilot\_v2.json} (its earlier control-free
version \texttt{results/llm\_decode\_pilot.json} is superseded and
retained only for diff inspection); Pilot~D (third-party library bypass;
100 library-shipped patterns $\to$ 48 baseline-matched $\to$ 48/48 bypass
across 8 of 9 libraries) is in \texttt{results/library\_pattern\_bypass.json}.
