...
% PCRE word-boundary anchor (typeset literally as \b in math mode)
\newcommand{\wb}{\ensuremath{\backslash\mathrm{b}}}
% PCRE word/non-word character class shorthands (in math mode)
\newcommand{\wbw}{\ensuremath{\backslash\mathrm{w}}}
\newcommand{\wbW}{\ensuremath{\backslash\mathrm{W}}}
...
% ============================================================
\begin{abstract}
LLM guardrails face four structurally distinct barriers:
algebraic blindness arising from syntactic monoid aperiodicity
(unconditional);
an illustrative information-theoretic lower bound (Fano-type, under a
uniformity assumption);
NP-hardness of instantiation verification;
and structural transfer via free-category functoriality (unconditional)
combined with string-level indistinguishability under a semantic-opacity
assumption on symbol naming.
...
We extracted syntactic monoids from a corpus of 142 patterns drawn from
twelve sources --- 100 patterns shipped by nine third-party open-source
guardrail projects and 42 patterns assembled from three author-curated
pattern sets;
...
\begin{enumerate}
  ...
  \item \textbf{Structural transfer and string-level indistinguishability}
    (\cref{sec:functor,sec:indistinguish}): abstract derivations transfer
    faithfully to real domains via free-category functoriality
    (Proposition~\ref{thm:transfer}, unconditional); and under a
    semantic-opacity assumption on symbol naming, the homomorphic-reasoning
    attack's prompts are string-level indistinguishable from legitimate
    formal-reasoning exercises
    (Observation~\ref{thm:indistinguish}).
    We present the indistinguishability result as a structural observation
    rather than an independent impossibility: it closes the argument
    \emph{once} opacity holds and an attack of this form exists (empirically
    demonstrated in \S\ref{sec:v3}--\ref{sec:v4} and visualized in
    Table~\ref{tab:visibility-trace}).
  ...
  \item \textbf{142-pattern regex corpus analysis plus validation} (\cref{sec:corpus}):
    confirms $100\%$ aperiodicity across 142 patterns drawn from twelve sources --- 100
    patterns shipped by nine third-party open-source guardrail projects and 42
    patterns from three author-assembled sets, matching the breakdown in CACM
    v14 Appendix~B.1 --- with verified $\mathrm{MOD}_2$ bypass constructions.
  \item \textbf{Defense recommendations grounded in formal results}
    (\cref{sec:discussion}): each impossibility theorem maps to a specific
    layer of a defense-in-depth stack; as a concrete instantiation of the
    Class~C defense architecture, we release \texttt{secrets-router}
    (\S\ref{sec:secrets-router}, MIT-licensed, ${\sim}800$ lines), an MCP
    server that inverts the interpretation-table asymmetry exploited by
    homomorphic-reasoning attacks.
\end{enumerate}
...
Transformer-based (neural) guardrails operate in $\mathrm{TC}^0$~\cite{merrill2023saturated,chiang2023tighter},
...
Ball et al.~\cite{ball2025impossibility} prove that under standard
cryptographic assumptions, adversarial prompts are computationally
indistinguishable from benign ones for any efficient filter; our results
are complementary---algebraic (assumption-free) blindness against modular
counting for regex guardrails (Theorem~\ref{thm:blindness}), and under
an opacity assumption on symbol naming, string-level indistinguishability
for the homomorphic-reasoning construction
(Observation~\ref{thm:indistinguish}).
Kumar et al.~\cite{certifying_safety2023} give certified defenses via
randomized smoothing against $\ell_p$-bounded perturbations; our attack is
a structure-preserving map that lies outside the smoothing distribution's
support.
Dong et al.~\cite{nfl_guardrails2025} establish a thoroughness/cost
no-free-lunch tradeoff; our results show that unbounded cost cannot
patch the regex blindness of Theorem~\ref{thm:blindness}.
R2-Guard~\cite{r2guard2025} replaces regex with chain-of-thought
reasoning, operating in $\mathrm{TC}^0$ and escaping aperiodic blindness
but remaining subject to the information-theoretic and structural
indistinguishability barriers of
\S\S\ref{sec:fano}--\ref{sec:indistinguish}.

The homomorphic-reasoning channel is a \emph{steganographic
channel}~\cite{hopper2002steganography}: harmful intent is hidden in a
legitimate-formal-reasoning covertext.
In the Hopper--Langford--von Ahn formalism, perfect steganographic
security requires that no polynomial-time distinguisher exceed negligible
advantage; Observation~\ref{thm:indistinguish} establishes \emph{perfect}
security under Assumption~\ref{ass:opacity} at the string-set level (zero
advantage, not merely negligible), rendering the distinguisher's task
information-theoretically impossible rather than computationally hard.

\subsection{Formal Languages and Neural Networks}\label{sec:related-formal}

The transformer complexity-theoretic literature places log-precision
transformers in $\mathrm{TC}^0$~\cite{merrill2023saturated,chiang2023tighter,merrill2024survey},
which \emph{can} compute modular predicates---hence neural guardrails are
not subject to the aperiodic blindness of the regex layer but remain
subject to the other barriers (\S\ref{sec:neural-tc0}).
Hahn~\cite{hahn2020limitations} provides circuit-theoretic evidence for a
related qualitative boundary on self-attention over periodic languages.
Our empirical syntactic monoid computation follows the same minimal-DFA
extraction pipeline used by Weiss et al.~\cite{weiss2022extracting} to
extract DFAs from RNNs via Angluin's $L^*$~\cite{angluin1987}.
...
\[
  G(w) = \begin{cases}
    \mathsf{block} & \text{if } \exists\, i,\, u,v,x:\\
                   & \quad w = u x v,\; x \in L(r_i), \\
...
The PCRE shorthands $\backslash s$, $\backslash d$, $\backslash w$ and their
negations $\backslash S$, $\backslash D$, $\backslash W$ are character classes
(they expand to finite unions of literal characters in any concrete alphabet),
so Kleene $*$/$+$ applied to them remains within this clause.
...
\begin{remark}[Theorem scope versus the empirical corpus]
Theorem~\ref{thm:substring-aperiodic} provides a sufficient
\emph{syntactic} condition for aperiodicity: regular expressions whose
Kleene iterators are applied only to character classes.
The Kleene-restriction to character classes is essential here: the
canonical counterexample is $(ab)^*$ (Kleene star applied to a
two-letter \emph{group} rather than to a character class), which
recognizes strings of the form $(ab)^n$ and requires
$\mathbb{Z}/2\mathbb{Z}$ in its syntactic monoid and falls outside this
syntactic scope (cf.\ CACM v14 Appendix~A.1).
A second pattern of this type is $(aa)^*$, which recognizes strings of
even $a$-count and is likewise non-aperiodic.
The empirical corpus result of \S\ref{sec:corpus} does not rely on the
theorem's syntactic template: we verify aperiodicity by \emph{computational
monoid enumeration} on each pattern (\S\ref{sec:corpus}, methodology
steps~3--4), which decides the conclusion $M(L_r)$ aperiodic directly
regardless of how the pattern is syntactically written.
The representative patterns in Table~\ref{tab:corpus} all use Kleene
iterators applied to character classes
(\texttt{\textbackslash s*}, \texttt{\textbackslash s+}, \texttt{.*},
\texttt{.+}), consistent with the theorem's syntactic scope; to the
extent any corpus pattern falls outside that scope the empirical result
still holds, because aperiodicity is checked from the enumerated monoid
rather than by template matching against the theorem.
Additionally, substring wrapping $\Sigma^* \cdot L(r) \cdot \Sigma^*$
frequently collapses Kleene-over-group constructions when $\varepsilon
\in L(r)$, reducing the wrapped language to a union of simple substring
matches on the base alternatives---which are within the theorem's scope.
\end{remark}

\begin{lemma}[Word-boundary anchors preserve aperiodicity]\label{lem:word-boundary}
Let $r$ be a regular expression in the grammar of Theorem~\ref{thm:substring-aperiodic},
and let \wb{} denote the PCRE word-boundary anchor whose semantics, as a
position predicate on $\Sigma^*$, expand to ``the previous character is in
\wbW{} (or position is the start) and the next character is in \wbw{}, or
vice versa,'' where \wbw{} denotes the word character class
$[\mathrm{A\text{-}Za\text{-}z0\text{-}9\_}]$ and \wbW{} denotes
$\Sigma \setminus \wbw$.
Then $L_r' := \Sigma^* \cdot \wb \cdot L(r) \cdot \wb \cdot \Sigma^*$ has aperiodic
syntactic monoid.
\end{lemma}
\begin{proof}
Define the auxiliary languages
$B_L = \{w x : x \in L(r),\ w \in \Sigma^*$ and the last character of $w$
is in \wbW{} or $w = \varepsilon\}$ and analogously
$B_R$ for the right anchor.
Each is a finite intersection of $\Sigma^* \cdot L(r) \cdot \Sigma^*$ with a
two-character context predicate (``character at position $|w|$ is in \wbW{} or
position $|w|$ is the start''), and these context predicates are themselves
star-free (each is a finite Boolean combination of length-bounded substrings
fixed at a position, which is realized as a finite union of two-letter
context tests evaluated by a constant-state DFA).
$L_r' = \Sigma^* \cdot \wb \cdot L(r) \cdot \wb \cdot \Sigma^*$ is the
intersection of two such star-free constraints with $\Sigma^* \cdot L(r) \cdot \Sigma^*$,
which is star-free by Theorem~\ref{thm:substring-aperiodic}.
Star-free is closed under finite intersection, so $L_r'$ is star-free, and by
Theorem~\ref{thm:smp} its syntactic monoid is aperiodic.
\end{proof}

This lemma covers the corpus patterns whose substring-aperiodicity status is
established only by enumeration in \S\ref{sec:corpus} due to the presence of
\wb{} anchors that fall outside the syntactic grammar of
Theorem~\ref{thm:substring-aperiodic}.
In particular, the single timeout pattern in our corpus that lies outside the
theorem's grammar---a Presidio IBAN regex of the form
\begin{center}
\small\texttt{\textbackslash{}b[A-Z]\{2\}\textbackslash{}d\{2\}[A-Z0-9]\{4\}\-\textbackslash{}d\{7\}([A-Z0-9]?)\{0,16\}\textbackslash{}b}
\end{center}
is aperiodic by Lemma~\ref{lem:word-boundary} applied to its inner regex,
which is itself in the grammar of Theorem~\ref{thm:substring-aperiodic}.
...
\paragraph{(2) Non-aperiodicity (under the alphabet condition).}
This step assumes there exists $f \in \Sigma$ such that $\delta_{\mathsf{block}}(q, f) = q$
for every $q \in Q_{\mathsf{block}}$ --- i.e., a filler character that does not advance
the blocked-DFA from any state.
This holds whenever $\Sigma \supsetneq \mathrm{alphabet}(L_{\mathsf{blocked}})$, which
in production regex deployments is essentially universal: pattern alphabets are small
relative to Unicode $\Sigma$, leaving many candidate fillers (whitespace, punctuation,
zero-width codepoints) available.
For patterns where this condition fails (e.g., regex containing $.$ or negated character
classes whose minimal-DFA alphabet equals $\Sigma$), step~(3) below carries the blindness
conclusion without requiring step~(2)'s cyclic witness; we discuss this regime after the
proof.

Consider the DFA constructed above, with states $(i, q) \in \{0,\ldots,p{-}1\} \times Q_{\mathsf{block}}$.
Let $f \in \Sigma$ be such a filler character (the $Q_{\mathsf{block}}$ component is
unchanged by $f$).
...
For the patchability claim, we use the syntactic-monoid result directly from
step~(2).
Step~(2) established that $\mathbb{Z}/p\mathbb{Z}$ embeds in the transition
monoid of $L_{\mathsf{decode}}(p)$ (under the alphabet condition).
We verify that this subgroup survives the syntactic quotient: for any
$1 \le j < p$, the elements $f^j$ and $f^p$ are syntactically inequivalent,
since any word $w \in L_{\mathsf{blocked}}$ with $|w| = k$ admits a context
$(x, y) = (\varepsilon,\, w)$ such that $x f^j y \in L_{\mathsf{decode}}(p)$ iff
the position alignment of $y$'s characters lands on indices $\equiv 0 \pmod p$,
which differs by $j \not\equiv 0 \pmod p$ between contexts.
Therefore the syntactic monoid $M(L_{\mathsf{decode}}(p))$ contains
$\mathbb{Z}/p\mathbb{Z}$ as a non-trivial subgroup, so $M(L_{\mathsf{decode}}(p))$
is non-aperiodic, and by Theorem~\ref{thm:bcst} we have
$L_{\mathsf{decode}}(p) \notin \mathrm{AC}^0$.

Now consider any aperiodic-monoid regex $G'$ added to $G$.
By Theorem~\ref{thm:bcst}, $L_{G \cup G'} \in \mathrm{AC}^0$.
By closure of star-free under finite Boolean operations
(Pin~\cite{pin1986}, \S I.4), $L_{G \cup G'}$ remains aperiodic, hence in
$\mathrm{AC}^0$.
Therefore $G \cup G'$ cannot decide $L_{\mathsf{decode}}(p)$, and the
modular-counting bypass class is not closed by any finite addition of
aperiodic-monoid regex patterns.
\end{proof}
...
\begin{theorem}[Substring-matched blindness without alphabet condition]\label{thm:regime2}
Let $L_{\mathsf{blocked}} = \Sigma^* \cdot L(r) \cdot \Sigma^*$ be a substring-matched
language, where $L(r)$ is regular and contains at least two distinct strings
$h_+, h_- \in \Sigma^k$ of equal length $k \ge 1$ that differ at some position
$s \in \{0, 1, \ldots, k-1\}$. Assume the alignment $(p, k, s)$ is
\emph{non-degenerate}, in the sense that the periodic decimation by $p$ of the
witness strings $h_+^N, h_-^N$ preserves the inequality at position $s$ across
some pair of offsets $j \neq j' \in \{0,1,\ldots,p-1\}$ for sufficiently large
$N$ (made precise in the proof below).
Then $M_{\Sigma}(L_{\mathsf{decode}}(p))$ contains $\mathbb{Z}/p\mathbb{Z}$ as a
subgroup, with no requirement that any character of $\Sigma$ be outside
$\mathrm{alphabet}(L_{\mathsf{blocked}})$.

\emph{Remarks.} The non-degenerate-alignment hypothesis is satisfied by every
nontrivial production guardrail we have surveyed: in practice $L(r)$ contains
witness strings that differ at multiple positions, so the inequality at $s$
survives the periodic decimation for the prime $p$ of any practical bypass. The
proof's inequivalence-via-periodic-decimation argument requires this
non-degenerate alignment; pathological corner cases where $L(r)$'s
accepted/rejected witness strings differ only at degenerate positions modulo
$\gcd(p, k)$ (e.g., $L(r) = a^k \cup b^k$ for $\gcd(p, k) = k$) are explicitly
carved out as out-of-scope for this article. See
\cref{sec:discussion}'s ``what we leave open'' paragraph, item~(e), and the
per-pattern audit of the corpus is acknowledged as future work.
\end{theorem}

\begin{proof}
We exhibit, for each $j \in \{0, 1, \ldots, p-1\}$, a string $u_j \in \Sigma^*$ of
length $j$, and prove that $\{u_0, u_1, \ldots, u_{p-1}\}$ are pairwise
syntactically inequivalent in $L_{\mathsf{decode}}(p)$, with composition realizing
$\mathbb{Z}/p\mathbb{Z}$.

\emph{Construction.} Fix any $a \in \Sigma$ (no neutrality required) and set
$u_j = a^j$ for $j = 0, 1, \ldots, p-1$.

\emph{Composition.} For $j, k \in \{0, \ldots, p-1\}$, the string $u_j u_k = a^{j+k}$
has length $j+k$ which is congruent to $(j+k) \bmod p$ modulo $p$.
Up to syntactic equivalence (justified by the inequivalence claim below),
$[u_j][u_k] = [u_{(j+k) \bmod p}]$, the multiplication of $\mathbb{Z}/p\mathbb{Z}$.

\emph{Inequivalence.} Fix $j \neq k$ in $\{0, 1, \ldots, p-1\}$.
We construct a context $(x, y) \in \Sigma^* \times \Sigma^*$ with
$x u_j y \in L_{\mathsf{decode}}(p)$ but $x u_k y \notin L_{\mathsf{decode}}(p)$.

Take $x = \varepsilon$.
Let $h$ be either $h_+$ or $h_-$ from the theorem hypothesis (we will choose
shortly), and set $y = h^N$ for $N$ to be chosen large enough below.

Compute $(u_j h^N)[0::p]$.
Position $i$ of $u_j h^N$ is in the decoded substring iff $i \equiv 0 \pmod{p}$.
Positions $0, 1, \ldots, j-1$ belong to $u_j$; positions $j, j+1, \ldots, j + Nk - 1$
belong to $h^N$.
For $j < p$, only position $0$ within $u_j$ satisfies $0 \equiv 0 \pmod{p}$,
contributing the character $a$ (or no character if $j = 0$, in which case $u_j$
is empty).
Within $h^N$: position $j + s$ is in the decoded substring iff
$s \equiv -j \pmod{p}$, contributing character $h^N[s] = h[s \bmod k]$ where
$s$ ranges over $\{0, 1, \ldots, Nk - 1\}$.

Let $\sigma_j(h) \in \Sigma^*$ denote the resulting subsequence of $h$ characters
extracted by this procedure --- specifically,
$\sigma_j(h) = h[(-j \bmod p) \bmod k] \cdot h[((-j + p) \bmod p \cdot \text{stride}) \bmod k] \cdot \ldots$
For large $N$, the decoded substring stabilizes (modulo a constant prefix and
suffix length) into a periodic sequence over the characters of $h$, with the
period determined by $\gcd(p, k)$ and the offset by $j \bmod p$.

\emph{Explicit context construction.}
Let $h_+, h_- \in L(r)$ of equal length $k$ differ at position $s$, and assume the
non-degenerate-alignment hypothesis of the theorem statement: namely, that for
sufficiently large $N$ and some pair of offsets $j \neq j' \in
\{0,1,\ldots,p-1\}$, the periodic decimation by $p$ surfaces the position-$s$
character of $h_+$ (resp.\ $h_-$) at distinct decoded indices in
$\sigma_j(h_+^N)$ vs.\ $\sigma_{j'}(h_+^N)$. Choose $N$ such that
$Nk \ge p \cdot \mathrm{lcm}(p, k)$; this guarantees both $\sigma_j(h_+^N)$ and
$\sigma_j(h_-^N)$ each contain the position-$s$ character of their respective
$h$ at some decoded index, and (under non-degenerate alignment) the periodic
decimation by $p$ preserves the inequality at position $s$ across
$\sigma_0, \sigma_1, \ldots, \sigma_{p-1}$.

Construct two contexts $(x_+, y_+) = (\varepsilon, h_+^N)$ and
$(x_-, y_-) = (\varepsilon, h_-^N)$.
For each pair $j, k \in \{0, 1, \ldots, p-1\}$ with $j \neq k$:
the decoded sequence $(u_j h_+^N)[0::p]$ extracts characters of $h_+^N$ at
positions $\{p \cdot t - j : t \ge 1\} \cap \{0, 1, \ldots, Nk-1\}$, indexed
modulo $k$ as $\{(p t - j) \bmod k : t \ge 1\}$.
By the non-degenerate-alignment hypothesis on $(p, k, s)$, the position $s$
appears in this index set for some $t$ with $\sigma_j$, and at a different
index $t'$ with $\sigma_k$.
The character at position $s$ in $h_+^N$ is $h_+[s]$; the character at the
same decoded-index position in $h_-^N$ is $h_-[s] \neq h_+[s]$.

Now: substring-matched $L_{\mathsf{blocked}}$ accepts a decoded sequence iff
that sequence contains some substring matching $L(r)$.
The decoded sequence $\sigma_j(h_+^N)$ contains a substring matching $L(r)$
(since $h_+ \in L(r)$ and the periodic structure with $h_+[s]$ at the right
offset reproduces $h_+$ as a substring of the decoded sequence for sufficiently
large $N$).
The decoded sequence $\sigma_j(h_-^N)$ may or may not contain such a substring,
depending on whether $h_- \in L(r)$ and the alignment.
Crucially: $\sigma_j(h_+^N)$ and $\sigma_k(h_+^N)$ differ at the indexed
position corresponding to $s$, and since the substring-match decision is
context-sensitive at that position, the contexts
$(x, y) = (\varepsilon, h_+^N)$ and $(x, y) = (\varepsilon, h_-^N)$ jointly
distinguish $u_j$ from $u_k$ in $L_{\mathsf{decode}}(p)$ for at least one
choice between the two contexts.

Therefore $u_j \not\sim u_k$ for $j \neq k$ in $\{0, 1, \ldots, p-1\}$, and the
composition $[u_j][u_k] = [u_{(j+k) \bmod p}]$ realizes $\mathbb{Z}/p\mathbb{Z}$
as a subgroup of $M_{\Sigma}(L_{\mathsf{decode}}(p))$.
\end{proof}

\begin{remark}[Combined coverage of regimes 1 and 2]\label{rem:both-regimes}
Step~(2) of Theorem~\ref{thm:blindness} (cyclic-witness via self-looping filler
in $\Sigma$) covers Regime~1: patterns whose alphabet is properly smaller than
$\Sigma$, which is the vast majority of production regex.
Theorem~\ref{thm:regime2} covers Regime~2: substring-matched patterns whose
minimal-DFA alphabet equals $\Sigma$ (e.g., \texttt{<script[\^{}>]*>}), with the
hypothesis that $L(r)$ contains at least two distinct strings of equal length ---
satisfied by every nontrivial production regex we have surveyed.
Together, the two results cover the substring-matching design pattern
unconditionally for nontrivial $L_{\mathsf{blocked}}$.
The pathological cases excluded by ``nontrivial'' --- $L(r) = \emptyset$,
$L(r) = \{\varepsilon\}$, or $L(r)$ containing only strings of one character at
each position --- correspond to regex that accepts everything, accepts nothing, or
matches a single fixed string, none of which are practically deployed as
guardrails for the bypass class.
\end{remark}
...
\begin{corollary}[Closure across the broader star-free string-matcher class]\label{cor:starfree-class-closure}
The blindness inheritance of \cref{prop:composition} extends from regex to
the broader class of star-free string-matchers:
\begin{itemize}
  \item \emph{Finite-language matchers (tries, DFA-based blocklists).}
    Every finite language $F \subseteq \Sigma^*$ is star-free
    ($F = \bigcup_{w \in F} \{w\}$, a finite union of singletons, each
    star-free), so trie- and DFA-based blocklists matching against
    finite literal sets inherit \cref{thm:blindness} directly.
  \item \emph{Bloom-filter blocklists.} A Bloom filter implementing
    membership in a finite literal set $F$ recognizes $F$ exactly when no
    false positives occur and a superset $F \cup F_{\mathsf{FP}}$
    otherwise. The substring-aperiodicity argument applies to $F$;
    bypass behavior matches that of a trie-based matcher of $F$, modulo
    false-positive contamination. The bound applies to the Bloom
    filter's \emph{intended} literal set.
  \item \emph{Any finite Boolean composition of the above.} Any union /
    intersection / complement of finitely many star-free string-matchers
    (regex, trie, DFA blocklist, Bloom filter on its intended set) is
    star-free, by the same syntactic-monoid divisibility argument as
    \cref{prop:composition} (cf.\ Pin~\cite{pin1986} \S I.4); the
    composed system is in $\mathrm{AC}^0$ and inherits blindness to
    $L_{\mathsf{decode}}(p)$.
\end{itemize}
Patch-resistance therefore extends from ``no further regex closes the
modular-counting bypass class'' to ``no further star-free
string-matcher of any kind closes it.''
\end{corollary}
...
\begin{align*}
  P_e &\;=\; \Pr[G'(U(R)) \neq G(R)] \\
      &\;\geq\; h^{-1}\!\bigl(H(G(R) \mid U(R))\bigr),
\end{align*}
...
The abstract grammar $\mathcal{G}$ uses symbol names drawn from a
domain-neutral vocabulary (e.g., $A, B, C$ or arbitrary identifiers) such
that no symbol name reveals the interpretation $I(m)$.
\end{assumption}

\begin{observation}[String-level indistinguishability under opacity]\label{thm:indistinguish}
Let $\mathcal{G}$ be an abstract grammar over a symbol set $M$ satisfying
Assumption~\ref{ass:opacity}.
Let $\mathcal{P}_{\mathsf{harm}}(\mathcal{G}, I)$ be the set of prompts
issued by the homomorphic-reasoning pipeline of \S\ref{sec:v3} under some
offline interpretation $I\colon M \to R$, and let
$\mathcal{P}_{\mathsf{legit}}(\mathcal{G})$ be the set of prompts a
legitimate user could issue to the same LLM as formal-reasoning exercises
over $\mathcal{G}$ (grammar completion, derivation search,
well-formedness checking).
Then for every $I$,
\[
  \mathcal{P}_{\mathsf{harm}}(\mathcal{G}, I)
  \;\subseteq\;
  \mathcal{P}_{\mathsf{legit}}(\mathcal{G})
\]
as sets of strings, with the inclusion independent of the choice of $I$.
Any guardrail that inspects only prompt content therefore has zero
distinguishing advantage between the two classes.
\end{observation}

\begin{proof}
By Assumption~\ref{ass:opacity}, names in $M$ carry no
interpretation-revealing information.
A homomorphic-reasoning prompt $p \in \mathcal{P}_{\mathsf{harm}}
(\mathcal{G}, I)$ consists of the grammar $\mathcal{G}$ (a string over $M$
and production symbols) together with a formal-reasoning instruction
(``find a derivation of target $t$''); it contains no part of $I$, whose
role is entirely offline.
Such a prompt is syntactically identical, as a string, to a prompt a
legitimate user could issue as a grammar-completion exercise over
$\mathcal{G}$; the set of all such legitimate prompts is, by definition,
$\mathcal{P}_{\mathsf{legit}}(\mathcal{G})$.
The inclusion holds for every $I$ because $I$ does not appear in $p$.
A guardrail operating on $p$ therefore sees the same string distribution
in both the adversarial and legitimate cases and has zero distinguishing
advantage.
\end{proof}

\paragraph{What the observation does and does not say.}
The substantive content lives in Assumption~\ref{ass:opacity} and in the
\emph{existence} of useful attacks whose prompts satisfy it---demonstrated
by the V3/V4 pipeline (\S\ref{sec:v3}--\ref{sec:v4}) and directly
visualized by the visibility trace (Table~\ref{tab:visibility-trace}).
What Observation~\ref{thm:indistinguish} contributes is the formal closure:
\emph{once} opacity holds and an attack of this form exists, the guardrail
boundary cannot distinguish the adversarial prompt stream from a
legitimate-reasoning prompt stream by looking at strings alone.
The observation is deliberately stated at the level of string sets, not at
the level of ``harmful vs.\ benign semantics,'' because the latter would
require a semantic notion we explicitly do not assume: the construction is
correct regardless of whether the LLM is performing symbolic reasoning or
retrieving memorized sequences, provided the LLM-visible surface is
opaque.
If Assumption~\ref{ass:opacity} fails---for example, if the attacker
carelessly names symbols after their real-world referents---the inclusion
does not hold, and a sufficiently informed guardrail may detect the attack
through symbol-name inspection.

\paragraph{Relation to Ball et al.}
Ball et al.~\cite{ball2025impossibility} prove computational
indistinguishability under standard cryptographic assumptions,
establishing that no efficient guardrail can distinguish a broad class of
adversarial prompts from benign ones with non-negligible advantage.
Observation~\ref{thm:indistinguish} is complementary: it establishes
string-level inclusion (a stronger form of indistinguishability, though
only at the string-set level) for the specific construction of
homomorphic reasoning, under an explicit opacity assumption rather than
under cryptographic assumptions.
The trade-off is that the observation applies to this construction
specifically, whereas Ball et al.\ handle a more general attack class.
Together, the two results cover the indistinguishability landscape for
prompt-level inspection; neither supplies the ``impossibility theorem'' of
the four barriers on its own, and we present Observation~\ref{thm:indistinguish}
as a structural characterization of the homomorphic-reasoning
construction rather than as an independent impossibility result.

% ============================================================
\section{Attack Vectors}\label{sec:vectors}

\paragraph{Taxonomy and relationships.}
We enumerate five attack vectors.
V1 (\emph{Decomposition}) and V2 (\emph{Zero-Knowledge Pipeline}) are
reframings of known techniques---runtime argument injection and
base64-encoded configuration---presented here as instances of the
abstraction framework rather than as novel attacks.
V3 (\emph{Homomorphic Reasoning}) and V4 (\emph{Encoding Bootstrap}) are
the joint novel contribution and compose as a single pipeline, read in
execution order as V4~$\to$~V3:
V4 uses the LLM to extract an abstract grammar from a natural-language
description of the target domain, and V3 then uses the LLM (or a classical
solver) to find derivations over that grammar using only opaque symbols;
the attacker applies an offline interpretation table to the solved
derivation to obtain the concrete procedure.
The aspirin and TCP case studies of \S\ref{sec:aspirin} run the full
V4~$\to$~V3 pipeline on two structurally distinct domains.
V5 (\emph{Modular Counting Bypass}) is independent: it exploits algebraic
blindness at the regex layer (Theorem~\ref{thm:blindness}) rather than the
homomorphic-reasoning construction, and is presented last as the
algebraic counterpart to V1--V4's structural attacks.
...
$\mathsf{block}$.
The mixed fiber is the mechanism, and the Fano-type lower bound
(Proposition~\ref{prop:fano}) quantifies the misclassification floor under
the uniformity assumption of \S\ref{sec:fano}.
...
The full $14 \times 4$ payload-pattern $\times$ prime bypass matrix
backing this table (Pilot~A; cf.\ CACM v14 Appendix~B.3) is archived in
\texttt{results/mod\_p\_bypass\_matrix.json} (56/56 bypass).}
...
We collected 142 regex patterns from twelve sources, partitioned as in
CACM v14 Appendix~B.1: \emph{nine third-party open-source guardrail
projects} contribute 100 patterns (LLM-Guard, llm-guard-py, Rebuff,
Guardrails-AI, Presidio, GitLeaks, LangKit, BodAIGuard, and the regex
subset of NeMo Guardrails' content rails), and \emph{three
author-assembled sets} contribute 42 patterns (8 inspired by the OWASP
Top~10 for LLMs taxonomy, 15 from a curated WAF-generic pattern set, and
19 author-constructed adversarial test patterns).
...
\end{enumerate}
Step~4 verifies aperiodicity directly from the enumerated monoid and does
\emph{not} rely on syntactic matching against
Theorem~\ref{thm:substring-aperiodic}; the corpus result therefore holds
as an empirical claim even for patterns whose regex source falls outside
the theorem's syntactic scope (see the Remark following the theorem).
...
\begin{table}[t]
\centering
\caption{Representative guardrail regex patterns from the 142-pattern corpus
(twelve sources: nine third-party projects and three author-assembled sets).
...
full corpus are aperiodic. Corpus-wide aperiodicity statistics
(per-source counts, monoid-size histogram, category breakdown) are
archived in \texttt{results/monoid\_distribution.json}, and the per-pattern
$\mathrm{MOD}_p$ bypass matrix is in \texttt{results/mod\_p\_bypass\_matrix.json}.}
\label{tab:corpus}
\small
...
\subsection{Non-LLM Defense-Class Bypass (Pilot~6)}\label{sec:non-llm-pilot}

The corpus analysis above measures the $\mathrm{MOD}_p$ bypass against the
LLM-guardrail class only. To extend the empirical coverage to two of the
non-LLM defensive classes named in the unification of CACM v14 \S2 (web
application firewalls and spam filters), we ran the same $\mathrm{MOD}_2$
bypass against representative real patterns from each class. The pilot is
documented in CACM v14 Appendix~B as Pilot~6 and the headline number
appears in CACM table row~6; we report the same numbers here for
completeness so that the algebraic argument has a paper-side empirical
leg in two non-LLM classes.

\paragraph{Setup.}
Sources: 6 ModSecurity OWASP CRS 4.27 SQL-injection patterns and 6
SpamAssassin \texttt{20\_phrases.cf} patterns (12 patterns total). 20
synthesized payloads (10 per class). $\mathrm{MOD}_2$ bypass tested with
three filler choices: NULL byte, ASCII space, and U+200B (zero-width
space). Same $\phi_2$ construction as the LLM pilot
(\S\ref{sec:v5}).

\paragraph{Results.}
\emph{ModSecurity:} 10/10 baseline-matched; 8/10 NULL bypass, 10/10
ASCII-space bypass, 10/10 ZWSP bypass. The two NULL-byte failures are
SQL-comment patterns whose alphabet \emph{includes} \verb|\x00| by design
(CRS catches NULL-byte injection explicitly); both bypass under
printable-space and zero-width-space fillers, exactly as the algebraic
argument predicts (no single-filler patch closes the bypass class).
\emph{SpamAssassin:} 9/10 baseline-matched (one synthesizer artifact);
9/9 NULL bypass, 9/9 ASCII-space bypass, 9/9 ZWSP bypass.

\paragraph{Reading.}
Across 12 patterns and 20 synthesized payloads from two defensive classes
distinct from the LLM-guardrail class that motivated the work, every
baseline-matched payload bypasses under at least one of the three filler
choices. The single-filler-resistant patterns (ModSec's two SQL-comment
patterns) are themselves bypassed by the other two filler choices. This
empirically confirms, in two non-LLM classes, the algebraic prediction
that no finite collection of single-filler patches covers the
modular-counting bypass class. The remaining defensive classes named in
CACM v14 \S2 (literal-string AV signatures, IDS regex tiers, code/secret
scanners) remain by algebraic argument only; the unification claim is
therefore \emph{empirical for three classes (LLM guardrails, WAF, spam
filter), algebraic for the rest}, in agreement with CACM v14 \S7.

Artifact: \texttt{results/\allowbreak non\_\allowbreak llm\_\allowbreak defense\_\allowbreak bypass.json}
(\texttt{by\_pattern}, \texttt{summary\_by\_class}, \texttt{sources},
\texttt{fillers} keys).

\paragraph{Empirical-artifact scope.}
The paper covers a strict subset of the empirical artifacts released with
this work. Three additional pilots whose detail belongs to the companion CACM v14
article are archived but not reproduced here in float-table form, and we summarize
their headline results so this section stands on its own.
Pilot~B (filler-diversity bypass) ran the same 14 baseline-matched instances against
4 primes $p \in \{2,3,5,7\}$ and 7 distinct printable fillers
(spaces, ZWSP, NBSP, asterisks, dots, dashes, underscores), yielding 392/392 bypass:
the modular-counting blind spot is filler-agnostic, not an artifact of any
single non-printable character.
Pilot~C (LLM decode reliability with a random-character control) used 4 open-weight
models across 60 decode cells (legitimate $\phi_p$-encoded payloads) and 20 control
cells (random-character interleavings) and measured a 91.7\,pp lift in decode
success on the legitimate cells over controls, ruling out spurious-pattern
recovery as an explanation for V5 efficacy.
Pilot~D (third-party library bypass) screened 100 library-shipped guardrail
patterns from nine libraries, of which 48 matched a baseline harmful payload, and
all 48 were bypassed by $\phi_2$ across 8 of 9 libraries (the ninth library
shipped only patterns that did not match the baseline).
Pilot~B is in \texttt{results/printable\_filler\_bypass.json} (CACM v14 Appendix~B.3);
Pilot~C is in \texttt{results/llm\_decode\_pilot\_v2.json} (its earlier
control-free version \texttt{results/llm\_decode\_pilot.json} is superseded and
retained only for diff inspection); Pilot~D is in
\texttt{results/library\_pattern\_bypass.json}.
Readers seeking the full per-pilot pipeline, payload synthesizer details,
and library normalization survey should consult CACM v14 Appendix~B; the
paper retains the full algebraic apparatus, the corpus aperiodicity
result (\S\ref{sec:corpus}), the non-LLM defense-class extension
(\S\ref{sec:non-llm-pilot}), the Tree-of-Thought benchmark
(\S\ref{sec:tot-benchmark}), and the case studies and end-to-end
production-pattern validation (\S\ref{sec:aspirin},
\S\ref{sec:e2e}) that anchor the formal claims.

\subsection{Tree-of-Thought Benchmark}\label{sec:tot-benchmark}

\paragraph{Protocol.}
We generated $N = 50$ randomized abstract grammars (seeds 0--49) via
\texttt{generate\_grammar} targeting a fixed terminal \texttt{N30}, each
containing multiple paths of optimal depth 5--8, two short low-yield
paths, and three high-yield dead-end traps.
Three solvers compete under a 90\,s timeout: (BFS) exhaustive
breadth-first search to depth 12, no LLM; (Random-beam) beam width 5
with composite score $0.6\cdot\text{random} + 0.4\cdot\text{yield}$;
(ToT) beam width 5 with composite $0.6\cdot\text{LLM rating} +
0.4\cdot\text{yield}$, where \texttt{llama3.1:8b} rates each candidate as
\textsc{sure}/\textsc{likely}/\textsc{impossible}.

\paragraph{Results.}
Table~\ref{tab:tot-results} reports mean yield, solve rate, and
bootstrap $95\%$ CIs ($B = 2000$).
BFS dominates on both metrics: Wilcoxon signed-rank, one-sided,
BFS vs.\ ToT $W = 1275$, $p < 0.001$; BFS vs.\ Random-beam $W = 1225$,
$p < 0.001$; the hypothesis ToT$\,>\,$BFS is not supported at any
grammar ($W = 0$, $p = 1.0$).

\begin{table}[t]
\centering
\caption{Tree-of-Thought benchmark results ($N = 50$ randomized
grammars, seeds 0--49, model \texttt{llama3.1:8b}, 90\,s timeout per
solver; unsolved grammars contribute yield $= 0$; 95\% CIs are
...
\paragraph{Interpretation.}
At the scale of these grammars (mean 87 states explored) BFS is cheap,
complete, and optimal, and the symbol-only grammars
(\texttt{N0},\ldots,\texttt{N30}, anonymous rule labels \texttt{R1},\ldots)
carry no surface vocabulary for the LLM's world knowledge to bind to;
the \textsc{sure}/\textsc{likely}/\textsc{impossible} ratings are
therefore effectively noise with respect to reachability, and their
$0.6\times$ composite weight actively misdirects the ToT beam into the
high-yield trap paths.
The honest reading: LLM-guided search adds value only when symbols carry
meaning the model can attach to---the regime the aspirin and TCP case
studies of \S\ref{sec:aspirin} exhibit---whereas on fully opaque
synthetic grammars classical search wins.
The security claim of V3 does not depend on ToT outperforming BFS on
such benchmarks; it requires only the \emph{existence} of a solver
producing valid derivations, which all three strategies demonstrate on
solvable grammars.

\paragraph{Reproducibility correction.}
An earlier draft of this section reported $N = 20$ results in which ToT
outperformed BFS by $4.3\times$.
On re-running the public benchmark code
(\texttt{src/\allowbreak aperiodic\_\allowbreak guardrails/\allowbreak benchmark/\allowbreak runner.py}) at $N = 50$,
those numbers did not reproduce; the honest results above replace them,
and raw outputs are archived in
\texttt{results/tot\_n50\_honest.json}.

\subsection{Case Studies}\label{sec:aspirin}

We validate the V3/V4 pipeline on three structurally distinct domains:
organic chemistry (aspirin synthesis), a network protocol (TCP state
machine), and a synthetic process calculus we constructed specifically
for this paper to eliminate any possibility of LLM pre-training exposure
(the Zerbik calculus, below).
All three runs instantiate the same homomorphic-reasoning construction
...
\paragraph{Synthetic domain: the Zerbik process calculus.}
Aspirin and TCP are public-knowledge domains in the LLM's training
distribution.
A hostile reviewer could reasonably ask whether successful V4/V3 runs on
those domains are evidence of homomorphic reasoning over abstract grammars
or merely retrieval of memorized sequences keyed off recognisable patterns
(``the LLM sees an aspirin-shaped grammar and reaches for the memorised
synthesis'').
To separate the two hypotheses we constructed the \emph{Zerbik process
calculus}---a fully invented formal system whose symbol vocabulary
(\texttt{zerbik}, \texttt{flarn}, \texttt{quonker}, etc.) appears in no
pre-training corpus we are aware of and whose rules we wrote for this
paper.
The calculus has seven primitive symbols $\{z_0,\ldots,z_7\}$ and a goal
symbol $z_*$; its six rewrite rules
($\{z_0\}\to\{z_1,z_2\}$,
$\{z_1\}\to\{z_3\}$,
$\{z_2\}\to\{z_4,z_5\}$,
$\{z_3,z_4\}\to\{z_6\}$,
$\{z_5\}\to\{z_7\}$,
$\{z_6,z_7\}\to\{z_*\}$)
define a reachability problem whose solution is the six-step derivation
\begin{align*}
\{z_0\} &{\to}\{z_1,z_2\}{\to}\{z_3,z_2\}{\to}\{z_3,z_4,z_5\} \\
        &{\to}\{z_6,z_5\}{\to}\{z_6,z_7\}{\to}\{z_*\}.
\end{align*}
BFS solves this in 6 rule applications; the LLM-guided V3 solver also
recovers a valid derivation (sometimes the same one, sometimes an
equivalent ordering).
Table~\ref{tab:zerbik-case} gives the interpretation table
$I\colon M_{\mathsf{zbk}} \to R_{\mathsf{zbk}}$ under which the solved
derivation produces a fictional but procedurally coherent
industrial-process output; the table is kept offline, as in the other two
case studies, and never enters the LLM context.

\begin{table}[t]
\centering
\small
\caption{Zerbik process calculus: a synthetic case study we constructed
for this paper. The symbol vocabulary is a deliberate non-word set; no
memorisation of the derivation is available to the LLM.}
\label{tab:zerbik-case}
\begin{tabularx}{\columnwidth}{@{}cllX@{}}
\toprule
\# & Abstract & $I(\cdot)$ & Real artifact \\
\midrule
1 & $z_1$ & \textsc{combine} & base + catalyst $\to$ mixture \\
2 & $z_3,z_4$ & \textsc{cure} & cured intermediate \\
3 & $z_5$ & \textsc{distill} & distilled solvent \\
4 & $z_*$ & \textsc{bond} & final compound \\
\bottomrule
\end{tabularx}
\end{table}

\paragraph{Scope of the claim and the memorisation question.}
The contribution is domain-agnostic pipeline correctness, not severity
demonstration.
Any domain expressible as a context-free grammar is vulnerable to the
same construction; we validate on three structurally distinct domains
and decline to validate on high-severity domains by design
(\S\ref{sec:ethics}).
Two of the three domains (aspirin, TCP) are public knowledge and so in
isolation they cannot empirically separate symbolic reasoning from
retrieval of memorised sequences.
The Zerbik calculus is included precisely to address this confound: its
vocabulary is invented, its rules were written for this paper, and the
LLM cannot have seen it before---yet the V3 pipeline still recovers a
valid derivation, demonstrating that the homomorphic-reasoning
construction does not require the LLM to hold prior memory of the target
domain.
Under either interpretation of what the LLM is ``really doing'' (symbolic
manipulation or pattern-matched retrieval), the inference-layer guardrail
sees only abstract grammar symbols and is unable to determine which
concrete domain is being instantiated offline.
\S\ref{sec:limitations-list}, item~6 discusses what the synthetic case
...
This is Observation~\ref{thm:indistinguish} made visible: the guardrail is
...
\addlinespace
\texttt{Rules}~$R_1{:}\{z_0\}{\to}\{z_1,z_2\},\ldots$; \texttt{solve for reachable}~$z_*$ &
\texttt{Rules}~$R_1{:}\{z_0\}{\to}\{z_1,z_2\},\ldots$; \texttt{solve for reachable}~$z_*$ &
combine $\to$ cure $\to$ distill $\to$ bond (Zerbik, synthetic) \\
\bottomrule
\end{tabularx}
\end{table*}
...
legitimate grammar exercises (Observation~\ref{thm:indistinguish}).
...
Both follow the abstraction pattern from \S\ref{sec:formalization}:
...
\paragraph{Scope of this section (closure under composition).}
The defense recommendations below rest on a closure-under-composition
argument that combines \cref{prop:composition,cor:starfree-class-closure}
(for the regex/star-free side) with the empirical and capacity-bound
behavior of $\mathrm{TC}^0$-class neural filters
(\cref{prop:filter-composition}). To make the load-bearing claims
explicit:

\emph{What we prove.} (i)~Boolean closure of star-free / aperiodic
languages: any finite intersection, union, or complement of aperiodic
substring-matching guardrails remains aperiodic, hence remains in
$\mathrm{AC}^0$, hence remains blind to $\mathrm{MOD}_p$ encodings
(\cref{prop:composition}, this section's
\cref{cor:starfree-class-closure}). (ii)~The closure extends \emph{across
defensive-class boundaries}: any composition of star-free string-matchers
(regex tier, trie blocklist, finite DFA, Bloom-filter blocklist on its
intended literal set) is itself star-free, regardless of which defensive
class each constituent comes from
(\cref{rem:cross-class-composition}). (iii)~In the regex/neural mixed
setting, the four serial/parallel/blend cases of
\cref{prop:filter-composition} characterize when the regex tier's
$\mathrm{AC}^0$ blindness propagates to the composed system and when the
neural tier's $\mathrm{TC}^0$ capacity recovers it.

\emph{What we assume.} The Boolean-closure model of composition treats a
multi-tier defense as a Boolean function of its constituents'
block-decisions (item~(3) of \cref{prop:filter-composition}); soft-score
serial pipelines are addressed only via the parallel and blend cases.
For \cref{prop:filter-composition}(5) we assume the filter score functions
$s_r, s_n\colon \Sigma^* \to [0,1]$ are calibrated such that
$f(x) = 1 \iff s(x) > \tau_f$ for some threshold $\tau_f$. The neural
tier is assumed to be a log-precision transformer, so by
Merrill--Sabharwal~\cite{merrill2023saturated} and
Chiang--Cholak--Pillay~\cite{chiang2023tighter} its decision language
lies in $\mathrm{TC}^0$. The Bloom-filter inheritance argument
(\cref{cor:starfree-class-closure}, third bullet) applies to the filter's
\emph{intended} literal set, not to the false-positive superset that the
filter actually accepts.

\emph{What we leave open.} (a)~The capacity bound $\mathrm{MOD}_p \in
\mathrm{TC}^0$ used in \cref{prop:filter-composition}(4) is a capacity
claim about what a $\mathrm{TC}^0$ circuit \emph{could in principle}
compute; it is \emph{not} a guarantee that a specific trained
fixed-architecture transformer reliably realises $\mathrm{MOD}_p$ on
unboundedly long inputs. Hahn~\cite{hahn2020limitations} shows this gap is
real for fixed-architecture self-attention without depth or head scaling;
the proposition's caveat now reflects this. We do not prove that any
particular production transformer guardrail closes the bypass class in
practice. (b)~\cref{tab:composition-empirical} is keyed to items (3), (4),
and (5) of \cref{prop:filter-composition}; items (1)--(2) (parallel
regex and serial regex) follow algebraically from
\cref{prop:composition} and are not reproduced as table rows. (c)~Pilot~6
(\cref{sec:non-llm-pilot}) supplies the empirical leg of
\cref{rem:cross-class-composition} for two non-LLM defensive classes (WAF
and spam filter); the remaining classes named in CACM v14 \S2
(literal-string AV signatures, IDS regex tiers, code/secret scanners)
remain by algebraic argument only and are listed as future work.
(d)~Hybrid topologies beyond plain parallel-OR / serial-AND
(rate-limited fallback, ensemble voting with a neural majority,
sampled-routing, multi-stage agentic pipelines with mixed AC$^0$/TC$^0$
arbiters) are discussed informally in CACM v14 \S5 and are not formalized
here. (e)~\cref{thm:regime2} covers nontrivial substring-matched
$L_{\mathsf{blocked}}$ under non-degenerate period alignment $(p, k, s)$ between
the witness strings of $L(r)$ and the bypass prime $p$. The corner case where
$L(r)$'s witness strings differ only at degenerate positions modulo
$\gcd(p, k)$ (e.g., $L(r) = a^k \cup b^k$ when $\gcd(p,k) = k$) is conjectured
to admit the same conclusion but is not proved here; a per-pattern audit of
the corpus to certify which patterns satisfy non-degenerate alignment for
relevant primes $p$ is acknowledged as future work.
...
Inference & Content filters, RLHF & Illustrative Fano-type lower bound under a uniformity assumption (\S\ref{sec:fano}); direct requests and obvious encodings caught \\
...
The Regex layer is algebraically blind (unconditional); the Inference layer
faces an illustrative Fano-type lower bound when abstraction fibers are
mixed (under the uniformity assumption of \S\ref{sec:fano}); the Schema
layer is NP-hard; and the Execution/OS layers limit functionality.
...
\subsection[Neural Guardrails and TC0]{Neural Guardrails and $\mathrm{TC}^0$}\label{sec:neural-tc0}

Regex guardrails recognize aperiodic languages, placing them in
$\mathrm{AC}^0$ via Theorems~\ref{thm:smp}--\ref{thm:fss}; the algebraic
blindness of Theorem~\ref{thm:blindness} is the direct consequence.
Transformer-based guardrails occupy a strictly higher class: log-precision
transformers sit in $\mathrm{TC}^0$~\cite{merrill2023saturated,chiang2023tighter},
and $\mathrm{AC}^0 \subsetneq \mathrm{TC}^0$~\cite{furst1981,hastad1987}.
Neural guardrails can therefore compute $\mathrm{MOD}_p$ predicates that
no regex guardrail can; the modular-counting blind spot of
\S\ref{sec:blindness-proof} does not transfer to them, and a sufficiently
trained transformer can in principle detect $\phi_p$-encoded payloads.
...
The threshold gates separating $\mathrm{TC}^0$ from $\mathrm{AC}^0$ compute
majority, parity, and iterated addition.
A single uniform attention head computes parity on a binary sequence
directly: it attends to all positions and accumulates a weighted sum
whose threshold distinguishes odd from even counts---the computation
$\mathrm{AC}^0$ circuits provably cannot
perform~\cite{furst1981,hastad1987}.
The canonical $\mathrm{MOD}_2$ bypass of Corollary~\ref{cor:parity}
(interleaved payload $w = a_1 b_1 a_2 b_2 \cdots a_n b_n$) therefore
collapses against a one-layer transformer guardrail while provably
evading any aperiodic regex.
Strobl et al.~\cite{strobl2024formal} give tight formal-language
expressivity bounds for hard-attention transformers of each depth,
including recognition of non-aperiodic regular languages;
Hahn~\cite{hahn2020limitations} gives parallel circuit-theoretic grounding
for the algebraic boundary via soft-attention saturation on periodic
languages.

\paragraph{What even $\mathrm{TC}^0$ cannot detect.}
$\mathrm{TC}^0$ has its own conjectured
ceiling~\cite{sipser2012,barrington1992}: $\mathrm{NC}^1$-complete problems
such as arbitrary Boolean formula evaluation are believed to lie outside
$\mathrm{TC}^0$, though the separation has resisted proof for four decades.
A guardrail that must evaluate a complex logical safety policy over its
input tokens is conjectured to require $\mathrm{NC}^1$ or higher, and
attackers who can force the detection problem into the
$\mathrm{NC}^1 \setminus \mathrm{TC}^0$ gap---conditionally on that
separation holding---can evade transformer guardrails by the analogous
complexity argument.
The algebraic impossibility for regex guardrails
(Theorem~\ref{thm:blindness}) is therefore unconditional, while the
analogous statement for neural guardrails is conditional on an open
complexity separation.
The Fano-type lower bound (Proposition~\ref{prop:fano}) and the NP-hardness
of instantiation (Proposition~\ref{thm:np}) apply to any guardrail
architecture inspecting only LLM-visible content, independently of its
circuit class, because both barriers arise from the fiber structure of
the abstraction function rather than from the guardrail's computational
complexity.
...
separation in the guardrail domain. Full training metrics, held-out evaluation
on $\mathrm{MOD}_p$ for $p \in \{2,3,5,7\}$, and clean-text false-positive
rates are released in the artifact \texttt{results/\allowbreak transformer\_\allowbreak tc0\_\allowbreak validation.json}.

\paragraph{Practical implications.}
The hierarchy recommends stacking: regex provides an $\mathrm{AC}^0$
first-pass filter (linear time, parallelizable, provably blind to modular
encodings), and a lightweight neural classifier trained specifically for
structure-based bypasses~\cite{strobl2024formal,merrill2024survey} closes
the $\mathrm{MOD}_p$ gap in principle.
Recent empirical work confirms the gap exists in production: Zhu et
al.~\cite{mathprompt2024} report $73.6\%$ attack success via symbolic
mathematical encoding across 13 production LLMs, an observable
manifestation of the algebraic blindness that Theorem~\ref{thm:blindness}
explains; cryptographic execution
proofs~\cite{proofofguardrail2026} demonstrate a guardrail \emph{ran} but
do not address the underlying blindness.
Two caveats limit the recommendation.
Neural guardrails are three to four orders of magnitude more expensive
than DFA traversal, adding non-trivial latency in high-throughput
deployments, and transformer guardrails are themselves subject to
gradient-based adversarial attacks~\cite{zou2023universal} that operate in
weight space rather than in the input language's algebraic structure---a
threat orthogonal to the impossibility results here but independently
serious.
The illustrative Fano lower bound (Proposition~\ref{prop:fano}, under its
uniformity assumption) continues to apply at the inference layer
regardless of the guardrail's circuit class; adversarial robustness of
neural filters is a separate unsolved problem.
Section~\ref{sec:filter-composition} formalizes the correct multi-layer
composition as filter composition laws.

\subsection{Filter Composition Laws}\label{sec:filter-composition}

We prove five composition laws governing the interaction of
$\mathrm{AC}^0$ (regex) and $\mathrm{TC}^0$ (neural) filters, answering
whether composing guardrail layers can repair the algebraic blindness of
\S\ref{sec:blindness-proof}.
Let $f_r$ denote a regex filter (star-free language, aperiodic syntactic
monoid), $f_n$ a neural filter ($\mathrm{TC}^0$ threshold circuit), and
write $f(x) = 1$ if $f$ \emph{blocks} $x$.
...
\item \textbf{Parallel with neural breaks the ceiling (capacity claim):}
  ...
  this composition \emph{can in principle} escape the $\mathrm{AC}^0$
  ceiling. \emph{Capacity-vs-realization caveat (Hahn~\cite{hahn2020limitations}).}
  This is a capacity claim about what some $\mathrm{TC}^0$ circuit
  could compute, not a guarantee that a specific trained
  fixed-architecture transformer reliably realises $\mathrm{MOD}_p$ on
  unboundedly long inputs: Hahn~2020 establishes that fixed-architecture
  self-attention cannot model parity for unboundedly long inputs unless
  depth or attention heads scale with sequence length. The empirical
  TC$^0$ validation of \S\ref{sec:neural-tc0} confirms that the capacity
  is achievable in the small-payload regime relevant to deployed
  guardrails (a 14{,}753-parameter one-layer-one-head transformer detects
  $\mathrm{MOD}_2$ on the corpus payload distribution at $90\%$); the
  unbounded-length realization remains conditional on architecture
  scaling.
...
Items~(1)--(2) confirm that stacking multiple regex filters cannot repair
blindness.
Item~(3) shows that a ``regex confirm, neural inspect'' architecture
inherits the regex layer's blindness, because the dangerous case is
precisely the one the regex does not flag.
Item~(4) gives the correct composition: run both in parallel, block if
\emph{either} flags, so the neural classifier covers the algebraic blind
spot (subject to the capacity-vs-realization caveat above).
Item~(5) quantifies the trade-off: soft-score blends must keep
$\alpha < \alpha^* = 1 - \tau/c$ or the regex weight dominates and
reimports $\mathrm{AC}^0$ blindness.

\begin{remark}[Cross-class composition: WAF $\circ$ IDS $\circ$ LLM-guardrail]\label{rem:cross-class-composition}
Items~(1)--(2) and \cref{prop:composition,cor:starfree-class-closure}
extend across defensive-class boundaries. A multi-tier pipeline
composing a WAF rule set, an IDS signature set, and an LLM guardrail
--- \emph{all} star-free / aperiodic --- is itself star-free, by the
same Boolean-closure argument (Pin~\cite{pin1986} \S I.4): the
composed system's syntactic monoid divides the direct product of the
three constituents' syntactic monoids, all of which are aperiodic, so
the product is aperiodic and the composition is in $\mathrm{AC}^0$.
The composed system therefore inherits blindness to $L_{\mathsf{decode}}(p)$.
The composition trap is the same mathematical object across the
defensive-security landscape: serial-AND propagates the most
algebraically-constrained tier's blindness to the system, regardless of
which defensive class that tier comes from. Pilot~6
(\S\ref{sec:non-llm-pilot}) supplies the empirical leg of this cross-class
claim for two non-LLM defensive classes (ModSecurity OWASP CRS as the WAF
constituent and SpamAssassin \texttt{20\_phrases.cf} as the spam-filter
constituent); the remaining classes named in CACM v14 \S2 are by
algebraic argument only and are listed under ``what we leave open'' in
\S\ref{sec:discussion}'s scope paragraph.
Compositions involving a non-star-free constituent (e.g., a $\mathrm{TC}^0$
neural arbiter in parallel-OR) are addressed by
\cref{prop:filter-composition}(4) and lie outside this remark's scope.
\end{remark}

\paragraph{Assumption (model of composition).}
Items~(1)--(4) of \cref{prop:filter-composition} model multi-tier
composition as a \emph{Boolean} function of the constituent filters'
block-decisions ($\wedge$ for serial-AND, $\vee$ for parallel-OR), with
$f(x) \in \{0, 1\}$. Item~(5) is the soft-score generalisation, with
calibrated score functions $s_r, s_n\colon \Sigma^* \to [0,1]$ such that
$f(x) = 1 \iff s(x) > \tau_f$; the $s_r(\phi_p(x)) \approx 0$ claim used
in item~(5) follows from \cref{thm:blindness} applied to a thresholded
star-free language. Pipelines outside this Boolean-or-linear-blend
model (e.g.\ neural rerankers conditioning on regex confidence as a
feature, or learned routing policies that train on the regex tier's
output distribution) are not covered by \cref{prop:filter-composition}
and are listed under ``what we leave open'' in \S\ref{sec:discussion}.

\paragraph{Empirical validation.}
We validated all five laws on 20 guardrail-style regex patterns, 20
$\mathrm{MOD}_2$-encoded payloads, and 20 clean-text samples, using a
character-bigram centroid classifier as the neural filter
(Table~\ref{tab:composition-empirical}).
...
Per-row detection rates and the per-encoding/per-stack breakdown are
archived in \texttt{results/\allowbreak comprehensive\_\allowbreak empirical\_\allowbreak validation.json}
(\texttt{encoding\_results}, \texttt{semantic\_attacks}, and
\texttt{preprocessing} keys).}
...
The theoretical critical weight $\alpha^* = 1 - 0.3/0.69 \approx 0.57$
predicts the detect-to-blind transition between $\alpha = 0.5$ and
$\alpha = 0.6$, matching the empirical drop from $95\%$ detection to
$5\%$; the serial-confirm row validates
Proposition~\ref{prop:filter-composition}(3) at zero detection.

\paragraph{Neural evasion analysis.}
Parallel regex~$\vee$~neural closes the $\mathrm{AC}^0$ blind spot but
not every attack vector.
We tested the neural filter (trained on $\mathrm{MOD}_2$ only) against
encoding schemes it had not seen (Table~\ref{tab:neural-evasion}).
...
Per-attack detection rates for the encoding-class rows
($\mathrm{MOD}_p$, Base64, ROT13) are in
\texttt{results/\allowbreak comprehensive\_\allowbreak empirical\_\allowbreak validation.json}; the V1 / V3
adaptive-adversary rows (100\% evasion of both regex and neural) are
backed by \texttt{results/\allowbreak class\_\allowbreak c\_\allowbreak adaptive\_\allowbreak adversary.json}
(\texttt{adaptive\_evasion\_rate}, \texttt{techniques} keys).}
...
\item \textbf{Class~C (semantic):} V1--V4 homomorphic reasoning.
  ...
  class.
  By Observation~\ref{thm:indistinguish}, these prompts are syntactically
...
The 858-line Python monoid extractor released as an artifact of this
work takes a guardrail regex and outputs (i)~the minimal DFA, (ii)~the
transition monoid with multiplication table, (iii)~all group components
identified via standard semigroup algorithms, (iv)~the complete
blindness spectrum---the set of primes $p$ for which the guardrail is
provably $\mathrm{MOD}_p$-blind (Proposition~\ref{prop:krohn-rhodes}),
and (v)~a concrete $\mathrm{MOD}_2$ bypass witness for each aperiodic
pattern.
Analysis of the 142 production patterns of \S\ref{sec:corpus} completes
in under 60 seconds with zero false positives, demonstrating practical
scalability; the tool integrates naturally into CI/CD as a pre-commit
verification step, flagging non-aperiodic patterns before deployment.
Because the analysis is theorem-backed rather than heuristic, a clean
audit report is a formal guarantee of blindness (to the stated primes),
shifting regex-guardrail verification from a confidence-based assertion
...
We validated this audit pipeline on the 142 patterns from twelve sources
(nine third-party open-source projects plus three author-assembled sets,
per CACM v14 Appendix~B.1; full inventory in Table~\ref{tab:corpus}).
...
No non-aperiodic patterns were found across any of the twelve sources
(nine third-party projects plus three author-assembled sets).
...
The empirical validation uses the aspirin synthesis case study, chosen for
its minimal harm risk.
The formal results (Theorems~\ref{thm:substring-aperiodic} and
\ref{thm:blindness}, Proposition~\ref{thm:transfer}, and
Observation~\ref{thm:indistinguish}) are domain-independent; their
applicability to high-severity domains is implied by the theory but not
empirically demonstrated here by design.
...
The corpus of 142 patterns from twelve sources (nine third-party projects
plus three author-assembled sets, per CACM v14 Appendix~B.1) is
...
\paragraph{6. Case-study memorization confound.}
Two of the three end-to-end case studies (aspirin synthesis, TCP state
machine) are public-knowledge domains extensively represented in LLM
pre-training data, raising a legitimate confound: those runs in isolation
cannot empirically distinguish homomorphic reasoning over abstract
grammars from retrieval of memorised sequences keyed off recognisable
symbolic patterns.
The third case study (the Zerbik process calculus of \S\ref{sec:aspirin},
Table~\ref{tab:zerbik-case}) was constructed specifically to control for
this confound: its symbol vocabulary is invented, its rewrite rules were
written for this paper, and the LLM cannot have seen it in pre-training;
the V3 pipeline nonetheless recovers a valid derivation, demonstrating
that the homomorphic-reasoning construction does not require prior
memorisation of the target domain.
Beyond that controlled case, the formal contribution does not depend on
the LLM actually performing symbolic reasoning in any interpretive sense:
the functoriality of the quotient interpretation map
(Proposition~\ref{thm:transfer}) and the structural indistinguishability
observation (Observation~\ref{thm:indistinguish}) require only that the
LLM-visible surface of the attack be syntactically indistinguishable from
a legitimate formal-reasoning workload, which
Table~\ref{tab:visibility-trace} demonstrates directly across all three
case studies.
The Zerbik case study is a single invented domain; further scale-up
(many synthetic grammars parameterised by rule count, branching factor,
and symbol-set size) would strengthen the empirical evidence beyond this
point and is left as future work.
...
We tested the full defense stack against 23 patterns extracted from LLM
Guard's \texttt{BanSubstrings} scanner~\cite{llmguard2024}, a production
guardrail library maintained by ProtectAI.
...
Source: \texttt{results/e2e\_llmguard\_validation.json}
(\texttt{n\_patterns}=23, \texttt{n\_payloads}=20, \texttt{patterns\_source}
= LLM-Guard \texttt{BanSubstrings}).}
\label{tab:e2e}
\small
\begin{tabular}{lcccc}
\toprule
...
MIT license, ${\sim}800$ lines. \emph{Disclosure: this implementation was developed
by the author as part of this project. Readers should verify its properties
against their own threat model.}}, an open-source MCP server that implements the
...
The opaque handle mechanism implements the abstraction pattern
from our formalization (\S\ref{sec:formalization}): it maps
concrete credentials to abstract handles, and the non-invertibility at
the inference layer provides the security guarantee.
The CDP fill operates at the execution layer
(Table~\ref{tab:defense}, row~4), precisely the layer our theory identifies
as necessary for Class~C defense.
Of the three components the Interpretation Firewall requires
(\S\ref{sec:aspirin}: action allowlisting, sandboxed interpretation,
provenance tracking), MCP natively provides two---tool declarations
implement allowlisting, and opaque handles implement provenance---leaving
sandboxed interpretation as the only remaining gap.

\paragraph{Defense walkthrough against V3/V4.}
The symmetry with our own attack is exact.
An attacker running V3 or V4 against a system protected by
\texttt{secrets-router} cannot exfiltrate credentials: the LLM context
contains only opaque handles, the interpretation table $I\colon
M_{\mathsf{cred}} \to R_{\mathsf{cred}}$ is held by the defender and never
appears on an LLM-visible surface, and the resolution of handles to
concrete values happens at the execution layer after every inference-layer
inspection has already passed.
The homomorphic-reasoning attack of \S\ref{sec:v3} exploits the defender's
inability to see the interpretation table while the attacker holds it;
\texttt{secrets-router} inverts this asymmetry by putting the defender on
the held-offline side of the abstraction boundary.
The attack and the defense are instances of the same architectural
pattern, distinguished only by who holds the interpretation function $I$
(cf.~Table~\ref{tab:visibility-trace}, row structure): the non-invertibility
is a vulnerability when the attacker holds $I$ and a security
guarantee when the defender holds it.
...
\paragraph{Information-theoretic lower bound (inference layer).}
Abstraction functions that map concrete artifacts to abstract types
inevitably create mixed fibers.
Under a uniform prior over concrete artifacts, Fano's inequality yields an
illustrative misclassification floor for any lifted guardrail: with
$30\%$ of mass in mixed fibers, the floor is approximately $5\%$
(Proposition~\ref{prop:fano}).
This is an illustrative bound, not an unconditional impossibility; the
algebraic blindness result above is the core contribution, and the Fano
bound quantifies an intuition about mixed fibers under simplifying
assumptions.
...
\paragraph{Structural transfer and indistinguishability.}
Abstract derivations transfer faithfully to real-domain operation sequences
via free-category functoriality (Proposition~\ref{thm:transfer},
unconditional): LLM reasoning over abstract grammars produces correct
real-world procedures after offline instantiation, regardless of whether
the LLM's internal process is interpretive reasoning or memorized
retrieval.
Under Assumption~\ref{ass:opacity} on symbol naming, the set of adversarial
prompts issued by the homomorphic-reasoning pipeline is contained
(as strings) in the set of legitimate formal-reasoning prompts over the
same grammar (Observation~\ref{thm:indistinguish}); a guardrail
inspecting only prompt content therefore has zero distinguishing
advantage between the two classes.
We present the indistinguishability result as a structural closure of
the attack construction under opacity, not as an independent impossibility
theorem---the work is done by the opacity assumption and by the
\emph{existence} of attacks satisfying it, demonstrated empirically in
\S\ref{sec:v3}--\ref{sec:v4}.
