...
(Proposition~\ref{prop:fano}, Theorem~\ref{thm:np}) that no inference-layer
...
The prototypical non-aperiodic monoid is the cyclic group
$(\mathbb{Z}/p\mathbb{Z}, +)$ for a prime $p \geq 2$: the generator $\bar{1}$
has additive order $p$, so $n \cdot \bar{1} \neq (n{+}1) \cdot \bar{1}$ for
$0 \leq n < p - 1$, and no power of $\bar{1}$ stabilizes before step $p$.
This cyclic behavior is precisely what aperiodic monoids cannot replicate.
...
The PoC implements all five attack vectors across five Python modules:
\texttt{engine.py} (78 lines: zero-knowledge recurrent executor for V1/V2),
\texttt{mediums/graph\_solver.py} (49 lines: BFS state-space solver),
\texttt{mediums/tot\_solver.py} (147 lines: Tree-of-Thought with LLM evaluation
for V3),
\texttt{mediums/web\_scraper.py} (80 lines: AST-based HTTP fetch),
and \texttt{encode\_config.py} (26 lines: config-to-base64 encoder).
A medium registry (\texttt{mediums/\_\_init\_\_.py}, 16 lines) selects the
execution medium at runtime.
The PoC totals 396 lines of Python---compact enough to fit within a single code
review session and below typical automated complexity thresholds.
Full source is provided in Appendix~\ref{app:poc}.
