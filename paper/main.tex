...
\begin{enumerate}
  ...
  \item \textbf{An open-source execution-layer defense}
    (\cref{sec:secrets-router}): \texttt{secrets-router}, an MIT-licensed
    MCP server that implements the Class~C defense architecture our theory
    recommends. Credentials are held outside the LLM context as opaque
    handles and resolved at the execution layer; the interpretation table
    is held by the defender, not the attacker, \emph{inverting} the
    asymmetry that enables the homomorphic-reasoning attack.
\end{enumerate}
