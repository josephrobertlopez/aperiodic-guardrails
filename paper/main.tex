...
The opaque handle mechanism is an implementation of the quotient functor $Q$
from our abstraction formalization (\S\ref{sec:formalization}): $Q$ maps
concrete credentials to abstract handles, and the non-invertibility of $Q$ at
the inference layer is the security guarantee.
...
the held-offline side of the same quotient functor.
The attack and the defense are instances of the same algebraic
construction, distinguished only by who holds $I$
(cf.~Table~\ref{tab:visibility-trace}, row structure): the non-invertibility
of $Q$ is a vulnerability when the attacker holds $I$ and a security
