...
\emph{This section requires no prior knowledge of formal language theory,
circuit complexity, or abstract algebra. Every concept used in the technical
sections is introduced here informally; the formal definitions follow in
\S\ref{sec:algebraic-foundations}.}

\subsection{The Gate}

A content filter reads a message and decides: safe or dangerous.
The filter has a list of patterns.
If the message matches any pattern, it is blocked.
This is how every major AI deployment protects itself at the first layer---a
pattern-matching gate between the user and the model.

Patterns are fast.
A single pass over the input, linear time, runs before the model processes
the request.
For the threats they were designed for---known keywords, obvious phrasings,
copy-pasted attack strings---they work.
The question this paper asks is: what are they \emph{structurally} unable to see?

\subsection{Inside the Machine}

A pattern like \texttt{bomb} compiles to a small machine that reads input one
character at a time, following arrows between states:

\begin{center}
$\bullet \xrightarrow{b} \circ \xrightarrow{o} \circ
\xrightarrow{m} \circ \xrightarrow{b} \bigstar$
\end{center}

If the machine reaches $\bigstar$, the message is blocked.
Otherwise it passes.
This machine is a \emph{deterministic finite automaton} (DFA).
It has a fixed number of states and no external memory---it knows where it
\emph{is}, but not where it has \emph{been}.

\subsection{The Fingerprint}

Every such machine has a mathematical fingerprint---a compact object that
captures everything the machine can and cannot distinguish.
Two messages that cause the machine to follow the same path are, from its
perspective, \emph{identical}.
It cannot tell them apart.
Not because it is poorly designed, but because the information needed to
distinguish them does not exist within its state space.

This fingerprint is the \emph{syntactic monoid}
(\S\ref{sec:syntactic-monoids})---the algebraic structure formed by
collapsing all indistinguishable inputs.

\subsection{The Lock That Never Unlocks}

Once a keyword machine matches \texttt{bomb} and reaches $\bigstar$,
it stays there forever:

\begin{center}
$\bigstar \xrightarrow{\text{any}} \bigstar
\xrightarrow{\text{any}} \bigstar
\xrightarrow{\text{any}} \bigstar \cdots$
\end{center}

Applying the same operation twice produces the same result as applying it once.
This property is called \emph{aperiodicity}: the machine reaches a fixed point
rather than cycling.
It is the algebraic signature of a machine that can match but cannot count.

A machine that \emph{can} count---such as one recognizing strings with an even
number of \texttt{a}'s---cycles between states:

\begin{center}
$\text{EVEN} \xrightarrow{a} \text{ODD} \xrightarrow{a} \text{EVEN}
\xrightarrow{a} \text{ODD} \cdots$
\end{center}

This machine loops.
Its fingerprint is \emph{periodic}---it has a cycle, and it can track parity.
Keyword filters have no such cycle.
They are deadbolts, not light switches.

\subsection{The Ceiling}

Three classical theorems, proved between 1965 and 1992, chain together to
establish the ceiling:

\begin{enumerate}
\item \textbf{Sch\"{u}tzenberger (1965):} A machine with an aperiodic
  fingerprint recognizes a \emph{star-free} language---one that can be defined
  using only AND, OR, NOT, and concatenation, without repetition counting.

\item \textbf{Barrington--Straubing--Th\'{e}rien (1992):} Star-free languages
  correspond exactly to $\mathrm{AC}^0$ circuits---constant-depth Boolean
  circuits with unbounded fan-in AND and OR gates.

\item \textbf{Furst--Saxe--Sipser (1981); H\aa{}stad (1987):} $\mathrm{AC}^0$
  circuits \emph{cannot compute parity}---they cannot determine whether a
  count is even or odd, regardless of circuit size.
\end{enumerate}

The chain:
keyword pattern $\to$ aperiodic fingerprint $\to$ star-free language $\to$
$\mathrm{AC}^0$ circuit $\to$ cannot count.
Each link is a published theorem.
The conclusion is not a conjecture; it is settled mathematics.

\subsection{The Interleaving Attack}

An attacker who knows the gate cannot count hides the payload using counting:

\begin{center}
\begin{tabular}{cccccccc}
\texttt{b} & \texttt{x} & \texttt{o} & \texttt{q} &
\texttt{m} & \texttt{w} & \texttt{b} & \texttt{k} \\
$\uparrow$ & & $\uparrow$ & & $\uparrow$ & & $\uparrow$ & \\
b & & o & & m & & b & \\
\end{tabular}
\end{center}

The payload occupies odd positions; the filler is random.
To decode, take every other character---a parity operation.
The gate operates in $\mathrm{AC}^0$.
$\mathrm{AC}^0$ cannot compute parity.
The gate passes the message.
It is algebraically blind.
...
\paragraph{Second domain: TCP state machine.}
To validate domain-agnosticism beyond chemistry, we applied the V4 grammar
extraction pipeline to the TCP connection state machine (RFC~793).
The grammar encodes: $S \to \text{SYN} \to \text{SYN-ACK} \to \text{ACK} \to
\text{ESTABLISHED} \to \text{FIN} \to \text{CLOSED}$.
When extracted via V4 into abstract symbols
($S \to t_1 \to t_2 \to t_3 \to t_4 \to t_5 \to t_6$) and solved via V3,
the derivation correctly reproduces the TCP three-way handshake and teardown
sequence after interpretation.
This confirms the domain-agnosticism claim across two structurally different
domains (chemical synthesis and network protocol) and demonstrates that
\emph{any} domain expressible as a context-free grammar is vulnerable to the
same homomorphic reasoning pipeline.
