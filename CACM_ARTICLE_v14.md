...
| Decade | Defensive class | Example bypass technique (citations) | Algebraic name |
|---|---|---|---|
| **1990s →** | Antivirus literal-string signatures (a *subset* of AV detection) | In-signature null-byte stuffing and no-op-byte interleaving against literal byte-pattern signatures. **Note:** classical *polymorphic* virus encoding works via crypto + decryption-stub mutation [Wikipedia 2026] — a *different* attack class that falls outside this bound. The MOD_p bound applies specifically to literal-substring AV signatures bypassed by in-pattern interleaving, not to encryption-based polymorphism. | MOD_p with no-op-byte filler |
| **mid-1990s →** | Web Application Firewalls (ModSecurity, AWS WAF, Cloudflare) | SQL-injection bypass via in-token whitespace, comment-character interleaving (`SEL/**/ECT`), Unicode normalization tricks [Awesome-WAF survey, 0xInfection 2024] | MOD_p with whitespace / comment-pair filler |
| **late 1990s →** | IDS/IPS regex tiers (Snort, Suricata) — *narrow subclass* | In-payload byte-interleaving against signature regex tiers when no protocol normalizer precedes them. **Note:** classical IDS evasion (Ptacek-Newsham 1998) exploits *parser-discrepancy* between IDS and endpoint TCP/IP stacks — a *different* attack class that falls outside this bound. The MOD_p bound applies specifically to the IDS regex/signature-matching tier when bypass content survives stream reassembly. | MOD_p against IDS regex tier (parser-discrepancy attacks excluded) |
| **early 2000s →** | Spam filters (SpamAssassin, regex-based content filters) | Acrostic spam, character-substitution + interleaving (`v.i.a.g.r.a`), Unicode-format-character evasion [Liu & Stamm 2007] | MOD_p with punctuation / format-char filler |
| **2010s →** | Source-code analysis, secret scanners (Semgrep, Bandit, GitLeaks, TruffleHog) | Comment-broken token bypass, secret splitting across line continuations | MOD_p with comment-token filler |
| **2020s** | LLM input/output guardrails (LLM-Guard, Presidio, Rebuff, Guardrails-AI, NeMo regex rails, OWASP-LLM patterns) | Zero-width Unicode insertion (Boucher 2022), payload splitting (Wei 2023), encoding-based jailbreak (Hackett 2025), uncommon text-organization (Li 2024) | MOD_p with zero-width / whitespace filler |
...
| **Network IDS regex tier scanning reassembled stream content** | Reassembled stream consumed by application protocol parsers; bypass relevant when content survives reassembly. **Note:** classical IDS evasion (Ptacek-Newsham 1998) uses parser-discrepancy between IDS and endpoint TCP/IP stacks — that attack class is *outside* this bound; the row here addresses only the IDS regex tier itself. | **Mixed → Low** (depends on protocol normalization; classical IDS evasion is the larger empirical class and lies outside our bound) |
...
| 2 | **Primary MOD_p bypass (LLM guardrails)** | 14 (payload, pattern) test instances at each prime — 12 payloads across 11 distinct patterns, with several payloads matching multiple patterns — × 4 primes (2, 3, 5, 7), NULL-byte filler | **56 / 56 bypass** |
| 3 | **Filler-diversity bypass (LLM guardrails)** | Same 14 test instances × 4 primes × 7 fillers (NULL, ASCII space, tab, tilde, backtick, U+200B, U+200C) | **392 / 392 bypass** |
| 4 | **Third-party LLM-library bypass** | 100 library-shipped patterns; 48 baseline-matched our literal-glue payload synthesizer across 8 of 9 libraries | **48 / 48 bypass** under both NULL and ASCII-space filler |
...
| 6 | **Non-LLM defense-class bypass** (extends the unification of §2 from algebraic to empirical for two more classes) | 6 ModSecurity OWASP CRS 4.27 SQL-injection patterns + 6 SpamAssassin `20_phrases.cf` patterns; 20 synthesized payloads; MOD_2 bypass tested with NULL, ASCII-space, U+200B fillers | **WAF (ModSec):** 10/10 baseline-matched; 8/10 NULL bypass (the 2 NULL failures are SQL-comment patterns whose alphabet *includes* `\x00` by design), 10/10 ASCII-space bypass, 10/10 ZWSP bypass. **Spam (SpamAssassin):** 9/10 baseline-matched (one synthesizer artifact); 9/9 NULL bypass, 9/9 ASCII-space bypass, 9/9 ZWSP bypass. **Multi-filler beats single-filler patches** — the NULL-resistant ModSec patterns bypass under printable filler, exactly as the algebraic argument predicts. |
...
Pilot 6 (the new non-LLM measurement) takes representative real patterns from two of §2's named classes — ModSecurity OWASP CRS for the WAF row, SpamAssassin's `20_phrases.cf` for the spam-filter row — and runs the same MOD_2 bypass. Across 12 patterns and 20 synthesized payloads, every baseline-matched payload bypasses under at least one of three filler choices (NULL, ASCII space, U+200B). The two NULL-byte failures (ModSec's SQL-comment patterns that explicitly catch `\x00`) bypass under printable-space and zero-width-space fillers — the algebraic prediction that no single-filler patch closes the bypass class is empirically confirmed *in a different defensive class than the one that motivated the work*.

The remaining defensive classes named in §2 (literal-string AV signatures, IDS regex tiers, code/secret scanners) are still by algebraic argument rather than by direct empirical pilot in this release. Extension to those classes is a clean follow-up; we expect the same pattern to hold given the algebraic structure is the same.
...
We do not claim our 142-pattern LLM corpus generalizes universally to the non-LLM defensive classes named in §2. We *do* present empirical extension to two non-LLM classes — ModSecurity OWASP CRS and SpamAssassin — in Pilot 6 (§4 row 6); the bypass succeeds across all baseline-matched payloads in both classes. The remaining defensive classes (literal-string AV signatures, IDS regex tiers, code/secret scanners) are by algebraic argument; the direct empirical sweep is a clean follow-up. The unification claim of §2 is therefore: *empirical for three classes (LLM guardrails, WAF, spam filter), algebraic for the rest.*

We are explicit about which §2 rows fall *outside* our bound's class even though they appear in the table to clarify the unification's edges: classical polymorphic-virus encoding (crypto + decryption-stub mutation) is a different attack class — the AV row addresses only literal-string AV signatures. Classical IDS evasion (Ptacek-Newsham 1998 parser-discrepancy attacks) is a different attack class — the IDS row addresses only the IDS regex tier itself.
...
16. Ptacek, T. H., and Newsham, T. N. (1998). Insertion, evasion, and denial of service: Eluding network intrusion detection. Technical Report, Secure Networks, Inc. *(Cited in §2 to distinguish parser-discrepancy IDS evasion from the algebraic regex-tier bypass class addressed in this work.)*
